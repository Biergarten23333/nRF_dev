...
def s2_log(reference:np.ndarray,directions:np.ndarray)->np.ndarray:
    ...
    cosine=np.clip(values@reference,-1.,1.)
    tangent=values-cosine[:,None]*reference
    norm=np.linalg.norm(tangent,axis=1);angles=np.arctan2(norm,cosine);scale=np.divide(angles,norm,out=np.ones_like(angles),where=norm>1e-12)
...
def fit_one(objective:Objective,start:np.ndarray,solver_cfg:Mapping[str,Any],jacobian_cfg:Mapping[str,Any],max_nfev:int):
    structure=objective.structural_sparsity(start);steps=parameter_steps(jacobian_cfg);limits=bounds()
    linear_solver=str(solver_cfg["tr_solver"])
    def production_jacobian(x):return approx_derivative(objective.residual,x,method="3-point",abs_step=steps,bounds=limits,sparsity=structure if linear_solver=="lsmr" else None)
    kwargs={}
    if linear_solver=="lsmr":kwargs["tr_options"]={"atol":float(solver_cfg["lsmr_atol"]),"btol":float(solver_cfg["lsmr_btol"]),"conlim":float(solver_cfg["lsmr_conlim"]),"maxiter":int(solver_cfg["lsmr_maxiter"])}
    tolerance=lambda name:None if solver_cfg[name] is None else float(solver_cfg[name])
    return least_squares(objective.residual,start,jac=production_jacobian,method="trf",tr_solver=linear_solver,loss=str(objective.cfg["loss"]),f_scale=float(objective.cfg["f_scale"]),bounds=limits,xtol=tolerance("xtol"),ftol=tolerance("ftol"),gtol=tolerance("gtol"),max_nfev=int(max_nfev),**kwargs)
