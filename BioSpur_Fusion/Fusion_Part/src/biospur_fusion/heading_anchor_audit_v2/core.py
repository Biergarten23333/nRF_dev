...
def production_reduced_factor_residual(edge: Mapping,
                                       ...
                                       measurement_protocol_relative: float) -> float:
    """Evaluate one modulo-pi factor using only protocol-relative K inputs."""
    if not isinstance(k_protocol_relative, KProtocolRelativeByCoordinate):
        raise TypeError(
            "production_reduced_factor_residual requires typed "
            "KProtocolRelativeByCoordinate"
        )
    if isinstance(measurement_protocol_relative, bool) or not math.isfinite(
        float(measurement_protocol_relative)
    ):
        raise ValueError("measurement_protocol_relative must be finite radians")
    endpoints = list(edge["endpoints"])
    kind = edge["factor_type"]
    if kind == "PROTOCOL_AXIS_LINE":
        segment = next(x for x in endpoints if x in k_protocol_relative)
        raw = (
            k_protocol_relative[segment]
            - float(measurement_protocol_relative)
        )
        ...
        raw = (
            k_protocol_relative[endpoints[0]]
            - k_protocol_relative[endpoints[1]]
            - float(measurement_protocol_relative)
        )
    else:
