...
FORMAL_REQUIRED_FIELDS = frozenset({
    "schema", "verdict", "heading_gauge_state",
    "heading_gauge_state_sha256", "semantic_cache_key",
})
FORMAL_ALLOWED_FIELDS = frozenset({
    *FORMAL_REQUIRED_FIELDS,
    "run_id", "source_commits", "selected_GF2_bit_vector",
    "selected_branch_count", "minimum_branch_margin",
    "candidate_payload_sha256", "production_mutation_count",
    "production_mutation_passed", "machine_gates", "support",
    "consumer_counts", "implementation_commit", "attestation_commit",
    "remote_commit",
})
FORMAL_SOURCE_COMMIT_FIELDS = frozenset({"r24_implementation", "r24_attestation"})
FORMAL_MACHINE_GATE_FIELDS = frozenset({
    "r24_audit_literal_checks_removed", "gf2_derived_from_actual_factor_behaviour",
    "I3_reassembled_not_copied", "all_production_mutations_executed",
    "pelvis_sign_golden_test_pass", "real_R_EiI_reference_loaded",
    "all_ten_horizontal_projections_nondegenerate",
    "all_512_actual_candidates_evaluated", "exactly_one_actual_branch_selected",
    "all_selected_branch_margins_positive",
    "seven_action_sign_crosschecks_no_conflict",
    "seven_action_sign_crosschecks_consistent", "forearm_selection_source_explicit",
    "all_joint_bootstrap_half_width_le_15deg", "axis_families_at_least_5_blocks",
    "heading_families_at_least_5_blocks",
    "opensense_common_heading_prerequisite_ready",
    "opensense_full_input_pipeline_ready", "phase4_ready",
    "sealed_consumer_count_zero", "deterministic_replay",
})
FORMAL_SUPPORT_FIELDS = frozenset({
    "schema", "resampling_unit", "frame_samples_treated_independent", "bootstrap",
    "within_donning_block_support", "between_donning_repeatability",
    "external_accuracy", "immediate_new_capture_required",
})
FORMAL_CONSUMER_COUNT_FIELDS = frozenset({
    "schema", "consumer_count", "numeric_consumer_count", "sealed_consumer_count",
    "forbidden_consumer_count", "counts_by_classification",
    "UWB_measurement_consumer_count", "OpenSense_consumer_count",
    "Vicon_consumer_count", "Phase4_consumer_count",
    "synthetic_truth_formal_consumer_count",
})
...
class FormalHeadingResult(TypedCanonicalPayload):
    ...
    @classmethod
    def from_json_bytes(
        cls, state: HeadingGaugeState, encoded: bytes
    ) -> FormalHeadingResult:
        if not isinstance(state, HeadingGaugeState) or not isinstance(encoded, bytes):
            raise TypeError("typed state and raw JSON bytes required")
        payload = _load_json_without_duplicate_keys(encoded)
        _validate_formal_heading_result_payload(state, payload)
        instance = object.__new__(cls)
        object.__setattr__(instance, "_heading_state", state)
        object.__setattr__(instance, "_payload_bytes", _canonical_json_bytes(payload))
...
def _load_json_without_duplicate_keys(encoded: bytes) -> Mapping[str, object]:
    def exact_object(pairs: list[tuple[str, object]]) -> dict[str, object]:
        result: dict[str, object] = {}
        for key, value in pairs:
            if key in result:
                raise HeadingGaugeValidationError(
                    f"duplicate raw JSON key refused: {key!r}"
                )
            result[key] = value
        return result

    try:
        payload = json.loads(encoded.decode("utf-8"), object_pairs_hook=exact_object)
    except (UnicodeDecodeError, json.JSONDecodeError) as exc:
        raise HeadingGaugeValidationError("formal result is not valid UTF-8 JSON") from exc
    if not isinstance(payload, Mapping):
        raise HeadingGaugeValidationError("formal result root must be an object")
    return payload
...
def _expect_exact_fields(
    ...
    *,
    ...
    if not isinstance(value, Mapping):
        raise HeadingGaugeValidationError(f"{field} must be an object")
    actual = set(value)
    ...
    if missing or unknown:
        raise HeadingGaugeValidationError(
            f"{field} fields are not exact; missing={missing}, unknown={unknown}"
        )
    return value
...
def _require_string(value: object, field: str) -> str:
    if not isinstance(value, str) or not value:
        raise HeadingGaugeValidationError(f"{field} must be a nonempty string")
    return value


def _require_bool(value: object, field: str) -> bool:
    if type(value) is not bool:
        raise HeadingGaugeValidationError(f"{field} must be boolean")
    return value


def _require_nonnegative_int(value: object, field: str) -> int:
    if type(value) is not int or value < 0:
        raise HeadingGaugeValidationError(f"{field} must be a nonnegative integer")
    return value
...
def _validate_formal_support(state: HeadingGaugeState, value: object) -> None:
    support = _expect_exact_fields(value, FORMAL_SUPPORT_FIELDS, "support")
    if support["schema"] != "biospur.phase3r26.support_bootstrap_gate_audit.v1":
        raise HeadingGaugeValidationError("support schema mismatch")
    _require_string(support["resampling_unit"], "support.resampling_unit")
    _require_bool(
        support["frame_samples_treated_independent"],
        "support.frame_samples_treated_independent",
    )
    _require_bool(
        support["immediate_new_capture_required"],
        "support.immediate_new_capture_required",
    )

    bootstrap_fields = frozenset({
        "source_sha256", "valid_replicates", "intervals",
        "single_heading_bootstrap_half_width_max_deg",
        "all_joint_bootstrap_half_width_le_15deg",
    })
    bootstrap = _expect_exact_fields(support["bootstrap"], bootstrap_fields, "support.bootstrap")
    _require_sha256(bootstrap["source_sha256"], "support.bootstrap.source_sha256")
    _require_nonnegative_int(bootstrap["valid_replicates"], "support.bootstrap.valid_replicates")
    _finite_number(
        bootstrap["single_heading_bootstrap_half_width_max_deg"],
        "support.bootstrap.single_heading_bootstrap_half_width_max_deg",
    )
    _require_bool(
        bootstrap["all_joint_bootstrap_half_width_le_15deg"],
        "support.bootstrap.all_joint_bootstrap_half_width_le_15deg",
    )
    intervals = bootstrap["intervals"]
    if not isinstance(intervals, Mapping) or not set(intervals) <= set(state.coordinate_order):
        raise HeadingGaugeValidationError("support bootstrap interval coordinates invalid")
    interval_fields = frozenset({
        "centre_rad", "half_width_rad", "half_width_deg", "replicates", "branch_fixed"
    })
    for coordinate, row_value in intervals.items():
        row = _expect_exact_fields(
            row_value, interval_fields, f"support.bootstrap.intervals.{coordinate}"
        )
        for field in ("centre_rad", "half_width_rad", "half_width_deg"):
            _finite_number(row[field], f"support.bootstrap.intervals.{coordinate}.{field}")
        _require_nonnegative_int(
            row["replicates"], f"support.bootstrap.intervals.{coordinate}.replicates"
        )
        _require_bool(
            row["branch_fixed"], f"support.bootstrap.intervals.{coordinate}.branch_fixed"
        )

    within_fields = frozenset({
        "families", "all_families_at_least_5_blocks", "deficient_family_count"
    })
    within = _expect_exact_fields(
        support["within_donning_block_support"], within_fields,
        "support.within_donning_block_support",
    )
    if not isinstance(within["families"], list):
        raise HeadingGaugeValidationError("support families must be a list")
    family_fields = frozenset({
        "family_id", "family_type", "existing_independent_block_count",
        "required_count", "deficit", "available_without_new_capture",
    })
    for index, row_value in enumerate(within["families"]):
        row = _expect_exact_fields(row_value, family_fields, f"support.families[{index}]")
        _require_string(row["family_id"], f"support.families[{index}].family_id")
        if row["family_type"] not in {"AXIS_FIT", "HEADING_FIT"}:
            raise HeadingGaugeValidationError("support family type mismatch")
        for field in ("existing_independent_block_count", "required_count", "deficit"):
            _require_nonnegative_int(row[field], f"support.families[{index}].{field}")
        _require_bool(
            row["available_without_new_capture"],
            f"support.families[{index}].available_without_new_capture",
        )
    _require_bool(
        within["all_families_at_least_5_blocks"],
        "support.within_donning_block_support.all_families_at_least_5_blocks",
    )
    _require_nonnegative_int(
        within["deficient_family_count"],
        "support.within_donning_block_support.deficient_family_count",
    )

    between_fields = frozenset({"independent_donning_count", "qualified", "note"})
    between = _expect_exact_fields(
        support["between_donning_repeatability"], between_fields,
        "support.between_donning_repeatability",
    )
    _require_nonnegative_int(
        between["independent_donning_count"],
        "support.between_donning_repeatability.independent_donning_count",
    )
    _require_bool(between["qualified"], "support.between_donning_repeatability.qualified")
    _require_string(between["note"], "support.between_donning_repeatability.note")

    external = _expect_exact_fields(
        support["external_accuracy"], frozenset({"available", "in_scope"}),
        "support.external_accuracy",
    )
    _require_bool(external["available"], "support.external_accuracy.available")
    _require_bool(external["in_scope"], "support.external_accuracy.in_scope")


def _validate_consumer_counts(value: object) -> None:
    counts = _expect_exact_fields(
        value, FORMAL_CONSUMER_COUNT_FIELDS, "consumer_counts"
    )
    if counts["schema"] != "biospur.phase3r26.data_access_summary.v1":
        raise HeadingGaugeValidationError("consumer-count schema mismatch")
    for field in FORMAL_CONSUMER_COUNT_FIELDS - {"schema", "counts_by_classification"}:
        _require_nonnegative_int(counts[field], f"consumer_counts.{field}")
    classifications = counts["counts_by_classification"]
    if not isinstance(classifications, Mapping) or any(
        not isinstance(key, str) or type(item) is not int or item < 0
        for key, item in classifications.items()
    ):
        raise HeadingGaugeValidationError("consumer classification counts invalid")
...
def _validate_formal_heading_result_payload(
    ...
    envelope = _expect_exact_fields(
        payload,
        FORMAL_REQUIRED_FIELDS,
        "formal result",
        allowed=FORMAL_ALLOWED_FIELDS,
    )
    _require_string(envelope["verdict"], "formal result verdict")
    if envelope["heading_gauge_state"] != state.to_payload():
        raise HeadingGaugeValidationError("formal result heading state mismatch")
    if envelope["heading_gauge_state_sha256"] != state.payload_sha256():
        raise HeadingGaugeValidationError("formal result heading-state SHA mismatch")
    if envelope["semantic_cache_key"] != state.semantic_cache_key:
        raise HeadingGaugeValidationError("formal result semantic key mismatch")

    if "run_id" in envelope:
        _require_string(envelope["run_id"], "formal result run_id")
    if "source_commits" in envelope:
        commits = _expect_exact_fields(
            envelope["source_commits"], FORMAL_SOURCE_COMMIT_FIELDS,
            "formal result source_commits",
        )
        for field in FORMAL_SOURCE_COMMIT_FIELDS:
            value = _require_string(commits[field], f"source_commits.{field}")
            if len(value) != 40 or any(character not in "0123456789abcdef" for character in value.lower()):
                raise HeadingGaugeValidationError(f"source_commits.{field} must be a commit SHA")
    if "selected_GF2_bit_vector" in envelope:
        bits = envelope["selected_GF2_bit_vector"]
        if bits is not None and (
            not isinstance(bits, list)
            or len(bits) != len(state.coordinate_order)
            or any(type(bit) is not int or bit not in (0, 1) for bit in bits)
        ):
            raise HeadingGaugeValidationError("selected GF2 bit vector invalid")
    for field in (
        "selected_branch_count", "production_mutation_count", "production_mutation_passed"
    ):
        if field in envelope:
            _require_nonnegative_int(envelope[field], f"formal result {field}")
    if "candidate_payload_sha256" in envelope:
        _require_optional_sha256(
            envelope["candidate_payload_sha256"], "candidate_payload_sha256"
        )
    if "minimum_branch_margin" in envelope and envelope["minimum_branch_margin"] is not None:
        node_fields = frozenset({
            "segment", "device", "target", "k_protocol_relative_rad",
            "psi_protocol_to_common_rad", "h_common_rad_derived", "h_common_derivation",
            "actual_reference_azimuth_rad", "candidate_axis_azimuth_in_P_rad",
            "directed_delta_rad", "primary_distance_rad", "primary_distance_deg",
            "antipodal_distance_rad", "antipodal_distance_deg", "margin_rad",
            "margin_deg", "preference",
        })
        margin = _expect_exact_fields(
            envelope["minimum_branch_margin"], node_fields,
            "formal result minimum_branch_margin",
        )
        for field in ("segment", "device", "target", "h_common_derivation", "preference"):
            _require_string(margin[field], f"minimum_branch_margin.{field}")
        for field in node_fields - {
            "segment", "device", "target", "h_common_derivation", "preference",
            "directed_delta_rad",
        }:
            _finite_number(margin[field], f"minimum_branch_margin.{field}")
        if margin["directed_delta_rad"] is not None:
            _finite_number(
                margin["directed_delta_rad"], "minimum_branch_margin.directed_delta_rad"
            )
    if "machine_gates" in envelope:
        gates = _expect_exact_fields(
            envelope["machine_gates"], FORMAL_MACHINE_GATE_FIELDS,
            "formal result machine_gates",
        )
        for field in FORMAL_MACHINE_GATE_FIELDS - {"deterministic_replay"}:
            _require_bool(gates[field], f"machine_gates.{field}")
        _require_string(gates["deterministic_replay"], "machine_gates.deterministic_replay")
    if "support" in envelope:
        _validate_formal_support(state, envelope["support"])
    if "consumer_counts" in envelope:
        _validate_consumer_counts(envelope["consumer_counts"])
    for field in ("implementation_commit", "attestation_commit", "remote_commit"):
        if field in envelope:
            _require_string(envelope[field], f"formal result {field}")
...
def _reject_untyped_heading_aliases(value: object, path: str = "payload") -> None:
    forbidden = {
        ...
        "heading_deg", "selected_heading_deg", "psi_GP_rad", "common_heading",
        "protocol_heading",
    }
