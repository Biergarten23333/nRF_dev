...
from .heading_types import KProtocolRelativeByCoordinate
...
def audit_repair(repo: Path, ledger: AccessLedger, report: Path,
    ...
    edges = graph["edges"]
    before = evaluate_reduced_graph(edges, base)
    tolerance = 1e-12
    rows = []
    invariant_bits = []
    for bits in itertools.product((0, 1), repeat=9):
        shifted = state.with_branch_bits(bits).k_protocol_relative_rad_by_coordinate
        after = evaluate_reduced_graph(edges, shifted)
...
def _score_k_space_branch_candidate(
    k_protocol_relative: KProtocolRelativeByCoordinate,
    reference: Mapping[str, float],
    bits: Sequence[int],
) -> dict:
    """Score one branch from an immutable K-only view."""
    if not isinstance(k_protocol_relative, KProtocolRelativeByCoordinate):
        raise TypeError("typed KProtocolRelativeByCoordinate required")
    if k_protocol_relative.coordinate_order != tuple(COORDINATE_ORDER):
        raise HeadingGaugeValidationError("K coordinate order mismatch")
...
def score_branch_candidate(
    ...
    """Pipeline dispatcher: K-only scoring, then one explicit K-to-H transport."""
    if not isinstance(state, HeadingGaugeState):
        raise TypeError("score_branch_candidate requires typed HeadingGaugeState")
    branch_state = state.with_branch_bits(bits)
    result = _score_k_space_branch_candidate(
        branch_state.k_protocol_relative_rad_by_coordinate,
        reference,
        bits,
    )
    enriched_nodes = []
    for node in result["per_node_directed_distance"]:
        segment = node["segment"]
        enriched_nodes.append({
            **node,
            "psi_protocol_to_common_rad": branch_state.psi_protocol_to_common_rad,
            "h_common_rad_derived": branch_state.h_common_rad(segment),
            "h_common_derivation":
                "wrap_2pi(k_protocol_relative_rad + psi_protocol_to_common_rad)",
        })
    return {
        **result,
        "per_node_directed_distance": enriched_nodes,
