...
from .model import ACTIONS,LAYOUT,Objective,_decode,canonical_calibration,fit_one,initial_parameters,migrate_spherical_latent_checkpoint,production_jacobian
...
def _soft_l1_cost(residual):return float(np.sum(np.sqrt(1.+np.square(residual))-1.))
...
def _derivative_audit(objective:Objective,x:np.ndarray,jac:np.ndarray,cfg:Mapping[str,Any])->dict:
    ...
    for ordinal in range(10):
        direction=rng.normal(size=len(x));direction/=np.linalg.norm(direction);fd=(objective.residual(x+step*direction)-objective.residual(x-step*direction))/(2*step);jv=jac@direction;relative=float(np.linalg.norm(fd-jv)/max(np.linalg.norm(fd),np.linalg.norm(jv),1e-12));r=objective.residual(x);analytic=float((jac.T@(r/np.sqrt(1.+r*r)))@direction);cost_fd=(_soft_l1_cost(objective.residual(x+step*direction))-_soft_l1_cost(objective.residual(x-step*direction)))/(2*step);cost_relative=abs(analytic-cost_fd)/max(abs(analytic),abs(cost_fd),1e-12);rows.append({"direction":ordinal,"jv_relative_error":relative,"soft_l1_cost_directional_relative_error":cost_relative})
    mask=objective.structural_sparsity(x).toarray().astype(bool);outside=float(np.max(np.abs(jac[~mask]))) if np.any(~mask) else 0.;passed=max(x["jv_relative_error"] for x in rows)<=float(cfg["maximum_jv_relative_error"]) and max(x["soft_l1_cost_directional_relative_error"] for x in rows)<=float(cfg["maximum_soft_l1_directional_cost_relative_error"]) and outside<=float(cfg["maximum_omitted_derivative_absolute"]);return {"schema":"biospur-production-endpoint-derivative-audit-v1","jacobian_method":"EXPLICIT_STRUCTURAL_SPARSITY_PLUS_3_POINT_CENTRAL_FD","structural_sparsity_nnz":int(mask.sum()),"dense_shape":list(mask.shape),"maximum_derivative_outside_declared_structure":outside,"directions":rows,"maximum_jv_relative_error":max(x["jv_relative_error"] for x in rows),"maximum_soft_l1_cost_directional_relative_error":max(x["soft_l1_cost_directional_relative_error"] for x in rows),"pass":passed}
...
def analyze(phase_a_dir:Path,template_path:Path,gates_path:Path,contract_path:Path,output:Path,resume_checkpoint:Path|None=None)->dict:
    ...
    if resume_checkpoint is not None:
        ...
        fit_start=migrate_spherical_latent_checkpoint(old_x);recomputed_cost=_soft_l1_cost(objective.residual(fit_start));cost_difference=abs(recomputed_cost-float(source["cost"]));cost_tolerance=1e-10*max(1.,abs(float(source["cost"])))
        resume_audit={"schema":"biospur-pole-chart-checkpoint-migration-v1","pass":cost_difference<=cost_tolerance,"source_checkpoint":str(resume_checkpoint.resolve()),"source_checkpoint_sha256":sha256(resume_checkpoint),"source_cache_sha256_verified":True,"action_segmentation_sha256_verified":True,"old_parameterization":"SPHERICAL_AZIMUTH_ELEVATION_WITH_POLE_SINGULARITY","new_parameterization":"REFERENCE_CENTERED_S2_TANGENT_CHART","source_soft_l1_cost":float(source["cost"]),"migrated_soft_l1_cost":recomputed_cost,"absolute_cost_difference":cost_difference,"cost_tolerance":cost_tolerance,"objective_rows":len(objective.residual(fit_start)),"physical_state_migration":"EXACT_S2_AXIS_PRESERVING","solver_state_reused":False}
    ...
    dump_json(output/"DETERMINISTIC_X0.json",{"schema":"biospur-attempt8-v2-deterministic-x0-v1","x0":x0.tolist(),"source":"initial_parameters(objective)","resume_forbidden":True})
    solver_start=time.perf_counter();fit=fit_one(objective,fit_start,gates["calibration_solver"],gates["production_jacobian"],fit_budget);first_done=time.perf_counter();dump_json(output/"START_0_CHECKPOINT.json",{"x":fit.x.tolist(),"cost":float(fit.cost),"optimality":float(fit.optimality),"nfev":int(fit.nfev),"status":int(fit.status),"success":bool(fit.success),"message":str(fit.message),"source_cache_sha256":sha256(cache),"gates_sha256":sha256(gates_path),"resumed_from":None,"single_start_only":True})
    jacobian_a=production_jacobian(objective,fit.x,gates["production_jacobian"]);jacobian_b=production_jacobian(objective,fit.x,gates["production_jacobian"])
    def csr_sha(value):
        digest=hashlib.sha256()
        for array in (value.data,value.indices,value.indptr):digest.update(str(array.dtype).encode());digest.update(np.asarray(array.shape,np.int64).tobytes());digest.update(np.ascontiguousarray(array).tobytes())
        return digest.hexdigest()
    jacobian_identity={"schema":"biospur-solver-audit-jacobian-identity-v1","solver_callback_implementation":"model.production_jacobian","audit_implementation":"model.production_jacobian","solver_callback_sha256":csr_sha(jacobian_a),"audit_sha256":csr_sha(jacobian_b),"same_structural_sparsity":True,"same_three_point_absolute_step":True,"pass":csr_sha(jacobian_a)==csr_sha(jacobian_b)};dump_json(output/"SOLVER_AUDIT_JACOBIAN_IDENTITY.json",jacobian_identity)
    jac=jacobian_a.toarray();sensitivity=_action_sensitivity(objective,fit.x,jac,gates["production_jacobian"]);dump_json(output/"ACTION_RESIDUAL_PARAMETER_SENSITIVITY.json",sensitivity)
    first_wall=first_done-solver_start;solver_ok=bool(fit.success and fit.optimality<=float(gates["calibration_solver"]["maximum_optimality"]));performance_ok=first_wall<=float(gates["calibration_solver"]["temporary_maximum_wall_time_s"]);wall=time.perf_counter()-solver_start;single={"schema":"biospur-attempt8-v2-single-start-audit-v1","original_deterministic_x0":True,"resume_used":False,"start_count":1,"success":bool(fit.success),"cost":float(fit.cost),"optimality":float(fit.optimality),"maximum_optimality":float(gates["calibration_solver"]["maximum_optimality"]),"nfev":int(fit.nfev),"status":int(fit.status),"message":str(fit.message),"wall_time_s":first_wall,"performance_pass":performance_ok,"solver_convergence_pass":solver_ok,"solver_audit_jacobian_identity":jacobian_identity["pass"],"eleven_action_sensitivity_pass":sensitivity["pass"],"multistart_started":False,"freeze_started":False,"replay_started":False,"render_started":False};dump_json(output/"SINGLE_START_AUDIT.json",single)
    result={"schema":"biospur-engineering-preview-attempt8-v2-single-start-result-v1","pass":False,"verdict":"SINGLE_START_CONVERGED_NEXT_STAGE_NOT_AUTHORIZED" if solver_ok else "FAIL_SINGLE_START_NOT_CONVERGED","single_start_convergence":solver_ok,"next_stage_authorized":False,"nonlinear_solver_started":True,"start_count":1,"solver":single,"golf":"SEALED","boxing":"SEALED","walk":"SEALED","final_still":"SEALED","uwb":"SEALED"};dump_json(output/"RESULT.json",result);return result

    qualification=False
    fits=[fit];multistart=[]
    if qualification:
        restart=fit_one(objective,fit.x,gates["calibration_solver"],int(gates["calibration_solver"]["restart_max_nfev"]));relative_cost=abs(float(restart.cost-fit.cost))/max(abs(float(fit.cost)),1e-12);qualification&=relative_cost<=float(gates["calibration_solver"]["maximum_restart_cost_relative_change"]);rng=np.random.default_rng(int(gates["calibration_solver"]["random_seed"]));starts=[x0]
