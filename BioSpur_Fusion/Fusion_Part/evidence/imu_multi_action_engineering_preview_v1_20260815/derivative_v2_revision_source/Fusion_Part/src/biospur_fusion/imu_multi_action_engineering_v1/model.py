...
def production_jacobian(objective:Objective,x:np.ndarray,jacobian_cfg:Mapping[str,Any],structure=None):
    if structure is None:structure=objective.structural_sparsity(x)
    dense=finite_difference_jacobian(objective.residual,x,parameter_steps(jacobian_cfg));mask=structure.toarray().astype(bool)
    return csr_matrix(np.where(mask,dense,0.))


def fit_one(objective:Objective,start:np.ndarray,solver_cfg:Mapping[str,Any],jacobian_cfg:Mapping[str,Any],max_nfev:int):
    structure=objective.structural_sparsity(start)
    callback=lambda x:production_jacobian(objective,x,jacobian_cfg,structure)
    options={"atol":float(solver_cfg["lsmr_atol"]),"btol":float(solver_cfg["lsmr_btol"]),"conlim":float(solver_cfg["lsmr_conlim"]),"maxiter":int(solver_cfg["lsmr_maxiter"])};return least_squares(objective.residual,start,jac=callback,method="trf",tr_solver="lsmr",tr_options=options,loss=str(objective.cfg["loss"]),f_scale=float(objective.cfg["f_scale"]),bounds=bounds(),xtol=float(solver_cfg["xtol"]),ftol=float(solver_cfg["ftol"]),gtol=float(solver_cfg["gtol"]),max_nfev=int(max_nfev))
