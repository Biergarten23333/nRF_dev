...
preregistered=tools/c2_full_session_continuous_delivery_preregistered.sha256
sha256sum -c "$preregistered" > "$output/PREREGISTERED_HASH_CHECK.txt" || exit 66
diagnostic=tools/diagnose_c2_full_session_continuous_delivery.py
diagnostic_sha=$(sha256sum "$diagnostic" | cut -d' ' -f1) || exit 66
child=(.venv-v0/bin/python -m tools.diagnose_c2_full_session_continuous_delivery
...
hash_inputs=(
    tools/run_c2_full_session_continuous_delivery_bounded.sh
    tools/diagnose_c2_full_session_continuous_delivery.py
    ...
    "$preregistered"
    tests/test_c2_full_session_continuous_delivery_harness.py
...
setsid /usr/bin/time -v -o "$output/RUNTIME.txt" \
    timeout --foreground --signal=TERM --kill-after=5s 11100 "${child[@]}" \
    > "$output/STDOUT.txt" 2> "$output/STDERR.txt" &
child_group=$!
printf '%s\n' "$child_group" > "$output/PROCESS_GROUP.txt"
wait "$child_group"
child_status=$?
...
size_gate=PASS
(( bytes <= 16777216 )) || size_gate=FAIL
structure_gate=FAIL
if [[ -s "$output/RESULT.json" ]] && \
    .venv-v0/bin/python - "$output/RESULT.json" <<'PY'
import json, sys
from tools.diagnose_c2_full_session_continuous_delivery import validate_result
