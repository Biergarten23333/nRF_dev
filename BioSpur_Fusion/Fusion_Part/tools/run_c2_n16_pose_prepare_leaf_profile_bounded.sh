...
preregistered=tools/c2_n16_pose_prepare_leaf_profile_preregistered.sha256
sha256sum -c "$preregistered" > "$output/PREREGISTERED_HASH_CHECK.txt" || exit 66
child=(.venv-v0/bin/python -m tools.profile_c2_n16_pose_prepare_leaf
...
hash_inputs=(
    tools/run_c2_n16_pose_prepare_leaf_profile_bounded.sh
    tools/profile_c2_n16_pose_prepare_leaf.py
    "$preregistered"
    tests/test_c2_full_session_ten_node_ab.py
    tests/test_c2_n16_pose_prepare_leaf_profile.py
...
setsid /usr/bin/time -v -o "$output/RUNTIME.txt" \
    timeout --foreground --signal=TERM --kill-after=5s 300 "${child[@]}" \
...
import json, sys
from tools.profile_c2_n16_pose_prepare_leaf import validate_bounded_result
...
validate_bounded_result(
    result,
    expected_profile_sha256=rows["tools/profile_c2_n16_pose_prepare_leaf.py"],
    expected_fixture_sha256=rows["tests/test_c2_full_session_ten_node_ab.py"],
    expected_pose_source_sha256=rows[
        "src/biospur_fusion/c2_uwb_calibration/causal_articulated_pose.py"
    ],
)
PY
