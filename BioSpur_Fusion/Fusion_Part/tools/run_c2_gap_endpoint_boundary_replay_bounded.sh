...
preregistered=tools/c2_gap_endpoint_boundary_replay_preregistered.sha256
sha256sum -c "$preregistered" > "$output/PREREGISTERED_HASH_CHECK.txt" || exit 66
diagnostic=tools/diagnose_c2_gap_endpoint_boundary_replay.py
diagnostic_sha=$(sha256sum "$diagnostic" | cut -d' ' -f1) || exit 66
child=(.venv-v0/bin/python -m tools.diagnose_c2_gap_endpoint_boundary_replay
...
hash_inputs=(
    tools/run_c2_gap_endpoint_boundary_replay_bounded.sh
    tools/diagnose_c2_gap_endpoint_boundary_replay.py
    ...
    "$preregistered"
    tests/test_c2_gap_endpoint_boundary_replay.py
...
import json, sys
from tools.diagnose_c2_gap_endpoint_boundary_replay import validate_result
