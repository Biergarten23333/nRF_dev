#!/usr/bin/env python3
"""Read-only BSF6C53 v47 cold-power persistence audit."""
...
def main() -> int:
    ...
    ap.add_argument("--observe-s",type=float,default=20);a=ap.parse_args();a.out_dir.mkdir(parents=True,exist_ok=False)
    ...
    with (a.out_dir/"fusion_cdc.log").open("x",encoding="utf-8",buffering=1) as log:
      try:
        ...
        try:
          if int(sf.get("up_ms","-1"),0)<0:failures.append("uptime unavailable")
