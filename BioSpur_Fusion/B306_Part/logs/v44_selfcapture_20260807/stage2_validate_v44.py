...
with (out / "fusion_cdc.log").open("x", encoding="utf-8", buffering=1) as log:
    ...
    try:
        ...
        def wait_back(timeout=180.0):
            """Wait for the board to GO AWAY, then come back.

            The previous version PINGed immediately and got a reply from the
            still-running pre-reset board, reported elapsed_s=0.1, and then
            checked for a corpse that had not been written yet. Absence has to
            be observed before presence means anything.
            """
            dl = time.monotonic() + timeout
            gone = False
            while time.monotonic() < dl:
                r = ask("PING", "PONG", timeout=8.0)
                if not r:
                    gone = True
                elif gone:
                    return r
        ...
        note("baseline STATUS", st is not None and "v44" in (st or ""), raw=st)
        ...
        if len(pages) == npages:
            ...
            # v44. The threshold is 20 s and the bracket now covers the HCI
            # EVENT arm, where a synchronous command can legitimately wait
            # HCI_CMD_TIMEOUT = 10 s. So the figure that matters is the worst
            # dwell of a NON-TERMINAL stage -- RX_WORK_ENTER above all, since
            # it is the one that spans a whole event dispatch. Terminal stages
            # (IDLE, RX_WORK_EXIT) measure idle time and are excluded.
            TERMINAL = {"IDLE", "RX_WORK_EXIT"}
            inflight = {k: v for k, v in mx_ms.items() if k not in TERMINAL}
            res["stage_max_inflight_ms"] = inflight
            res["rx_work_enter_ms"] = inflight.get("RX_WORK_ENTER")
            worst = max(inflight.values()) if inflight else 0.0
            res["worst_inflight_stage"] = max(inflight, key=inflight.get) if inflight else None
            res["stage_max_worst_ms"] = worst
            res["threshold_margin_x"] = round(20000.0 / worst, 1) if worst else None
            note("healthy dwell measured (in-flight only)", bool(inflight),
                 worst_stage=res["worst_inflight_stage"], worst_ms=worst,
                 rx_work_enter_ms=res["rx_work_enter_ms"],
                 margin_x=res["threshold_margin_x"])
            note("worst in-flight dwell leaves margin on the 20 s threshold",
                 worst < 10000.0,
                 note_="if this approaches 10 s the threshold must rise again")
