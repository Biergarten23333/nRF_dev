...
/* BSS initialization is the reboot-time reset. Runtime CFG must never reset it. */
	...
	struct ss_twr_init_beacon_status *status)
{
	...
	status->beacon_rx_arm_failures =
		ss_twr_init_beacon.rx_arm_failures;
#if APP_TAG_RELAY6_COUNTER_ENABLE != 0U
...
static void ss_twr_init_beacon_reset(void)
{
	...
	tag_relay8_2_tracker_reset(&ss_twr_init_beacon.tracker);
				      ...
				      uint64_t rx_ts40)
{
	...
	generation_changed = ss_twr_init_beacon.locked &&
	...
	tag_relay8_2_tracker_accept(
		&ss_twr_init_beacon.tracker, origin40,
		payload.superframe_counter, payload.cycle_period_us,
		generation_changed);
	ss_twr_init_beacon.next_window_origin40 =
		ss_twr_init_beacon.tracker.next_origin40;
	return 2;
}
...
static uint32_t ss_twr_init_beacon_window_early_us(void)
{
	return tag_relay8_2_window_early_us(
		ss_twr_init_beacon.tracker.epochs_since_valid);
}

static uint32_t ss_twr_init_beacon_window_close_us(void)
{
	return tag_relay8_2_window_close_us(
		ss_twr_init_beacon.tracker.epochs_since_valid);
}

static void ss_twr_init_beacon_restore_radio_defaults(void)
{
	dwt_forcetrxoff();
	...
	ss_twr_init_beacon.rx_armed = false;
}

static bool ss_twr_init_beacon_listen_until(uint64_t deadline40);

/*
 * Arm the next beacon window in DW hardware immediately after the ranging
 * radio tail. The CPU may format/publish the completed sweep while DX_TIME
 * counts down; it is no longer responsible for arriving at the window edge.
 */
static bool ss_twr_init_beacon_arm_next_window(void)
{
	uint64_t now40;
	...
	if (!ss_twr_init_runtime_params.beacon_sync ||
	    !ss_twr_init_beacon.locked || ss_twr_init_beacon.rx_armed ||
	    ss_twr_init_runtime_update_pending) {
		return false;
	}

	early_us = ss_twr_init_beacon_window_early_us();
	close_us = ss_twr_init_beacon_window_close_us();
	window_start = uwb_beacon_sub40(
		ss_twr_init_beacon.next_window_origin40,
		uwb_beacon_us_to_dw_ticks(early_us));
	window_close = uwb_beacon_add40(
		ss_twr_init_beacon.next_window_origin40,
		uwb_beacon_us_to_dw_ticks(close_us));
	now40 = ss_twr_init_dw_now40();
	if (uwb_beacon_diff40(window_start, now40) <= 0) {
		ss_twr_init_beacon.rx_arm_failures++;
		return false;
	}

	window_us = early_us + close_us;
	dwt_forcetrxoff();
	dwt_write32bitreg(SYS_STATUS_ID,
			  SYS_STATUS_ALL_RX_GOOD | SYS_STATUS_ALL_RX_ERR |
			  SYS_STATUS_ALL_RX_TO);
	dwt_setdelayedtrxtime((uint32_t)(window_start >> 8));
	dwt_setrxtimeout((uint16_t)MIN(window_us, UINT16_MAX));
	rc = dwt_rxenable(DWT_START_RX_DELAYED);
	if (rc != DWT_SUCCESS) {
		ss_twr_init_beacon.rx_arm_failures++;
		ss_twr_init_beacon_restore_radio_defaults();
		return false;
	}

	ss_twr_init_beacon.armed_window_close40 = window_close;
	ss_twr_init_beacon.rx_armed = true;
	return true;
}

static bool ss_twr_init_beacon_finish_armed_window(void)
{
	uint32_t status_reg;
	bool accepted = false;
	bool continue_immediate = false;
	uint64_t deadline40 = ss_twr_init_beacon.armed_window_close40;

	if (!ss_twr_init_beacon.rx_armed) {
		return false;
	}
	...
	if ((status_reg & SYS_STATUS_RXFCG) != 0U) {
		uint32_t frame_len = dwt_read32bitreg(RX_FINFO_ID) &
		...
		if (frame_len <= sizeof(ss_twr_init_rx_buffer)) {
			...
			accepted = ss_twr_init_consume_beacon(
				ss_twr_init_rx_buffer, frame_len,
				bsl_ts40_get(rx_ts_bytes)) == 2;
		}
		continue_immediate = !accepted &&
			uwb_beacon_diff40(deadline40,
					  ss_twr_init_dw_now40()) > 0;
	}
	...
	uint32_t configured_period_us)
{
	...
	tag_relay8_2_tracker_coast(
		&ss_twr_init_beacon.tracker, configured_period_us);
	ss_twr_init_beacon.next_window_origin40 =
		ss_twr_init_beacon.tracker.next_origin40;
}
...
static void ss_twr_init_beacon_listen_if_needed(void)
{
	...
	if (ss_twr_init_beacon.locked) {
		...
		if (ss_twr_init_beacon.rx_armed) {
			accepted = ss_twr_init_beacon_finish_armed_window();
			if (!accepted) {
		...
		close_us = ss_twr_init_beacon_window_close_us();
		while (uwb_beacon_diff40(
				now40, uwb_beacon_add40(
					ss_twr_init_beacon.next_window_origin40,
					uwb_beacon_us_to_dw_ticks(close_us))) >= 0) {
			ss_twr_init_beacon_note_missed_window(
				configured_period_us);
			close_us = ss_twr_init_beacon_window_close_us();
		}
		...
		early_us = ss_twr_init_beacon_window_early_us();
		if (uwb_beacon_diff40(
				uwb_beacon_sub40(
					ss_twr_init_beacon.next_window_origin40,
					uwb_beacon_us_to_dw_ticks(early_us)),
				now40) >
		    (int64_t)uwb_beacon_us_to_dw_ticks(
			    configured_period_us + close_us)) {
			return;
		}
		window_start = uwb_beacon_sub40(
			ss_twr_init_beacon.next_window_origin40,
		...
		window_close = uwb_beacon_add40(
			...
			uwb_beacon_us_to_dw_ticks(close_us));
		if (uwb_beacon_diff40(window_start, now40) > 0 &&
		    ss_twr_init_beacon_arm_next_window()) {
			accepted = ss_twr_init_beacon_finish_armed_window();
		} else {
    ...
    const struct uwb_tag_runtime_config *config)
{
    ...
    ss_twr_init_anchor_count = config->anchor_count;
	    /* Reboot-time BSS initialization owns the one permitted reset. */
    ss_twr_init_active_anchor_count = 0U;
    ss_twr_init_active_anchor_index = 0U;
    ss_twr_init_current_anchor_retry_count = 0U;
...
#endif
#endif
    (void)ss_twr_init_beacon_arm_next_window();
    if (cir_diag_pending) {
