...
/* ------------------------------------------------------------------------ */
/* Relay command/ack frames (B306 <-> DWM1001C, opposite UART directions)   */
/* ------------------------------------------------------------------------ */

/*
 * The fixed 96-byte BSL data frame below remains byte-identical. Relay
 * traffic uses a different magic and an explicit type, so a parser cannot
 * mistake a variable-length command/ack for ranging data.
 *
 * Wire layout:
 *   bsl_relay_hdr_t + payload[len] + uint16_t CRC-16/CCITT-FALSE
 *
 * CRC covers header and payload. Payload is ASCII without a trailing NUL.
 * correlation is generated by B306 and copied unchanged into the tag ack.
 */
#define BSL_RELAY_MAGIC0       0xC3u
#define BSL_RELAY_MAGIC1       0x6Du
#define BSL_RELAY_VERSION      1u
#define BSL_RELAY_TYPE_COMMAND 1u
#define BSL_RELAY_TYPE_ACK     2u
#define BSL_RELAY_PAYLOAD_MAX  191u
...
#define BSL_RELAY_FRAME_OVERHEAD \
	((uint32_t)sizeof(bsl_relay_hdr_t) + (uint32_t)sizeof(uint16_t))
#define BSL_RELAY_FRAME_MAX \
	(BSL_RELAY_FRAME_OVERHEAD + BSL_RELAY_PAYLOAD_MAX)

_Static_assert(sizeof(bsl_relay_hdr_t) == 8u,
	       "biospur_link: relay header size drifted");
_Static_assert(BSL_RELAY_PAYLOAD_MAX >= 160u,
	       "biospur_link: relay CFG capacity is too small");
_Static_assert(BSL_RELAY_PAYLOAD_MAX <= 191u,
	       "biospur_link: relay payload exceeds tag command buffer");
