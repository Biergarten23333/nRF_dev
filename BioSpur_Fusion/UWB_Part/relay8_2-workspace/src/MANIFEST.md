# MANIFEST — firmware source provenance

This directory began as the exact snapshot described below and is now the
writable Task A derivative. The current differences from the frozen snapshot
are intentionally limited to:

- tag UART0 conversion to 460800-baud EasyDMA UARTE and non-blocking RTT;
- shared `biospur_link.h` v2 and the asynchronous UART/strobe implementation;
- v2 frame population, including effective identity and measured
  `t_round_us[]`;
- switchable BLE `TR;` output, long-interval capture mode, link counters, and
  UART/strobe quiescence during OTA.

`SHA256SUMS.txt` records the imported baseline and is not a checksum manifest
for the modified Task A sources. Use the repository diff and
`../TASK_A_REPORT.md` for the current change set.
...
**Imported exactness:** before Task A edits, no firmware source file had changed
after `freeze-clean-20260716`; the imported tree was byte-identical to the
source that produced the frozen binaries in `../`. Baseline integrity:
`SHA256SUMS.txt` (150 files).
