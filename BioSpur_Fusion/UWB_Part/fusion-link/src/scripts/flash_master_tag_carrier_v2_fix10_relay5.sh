...
# Destructive, one-shot-per-authorized-batch relay5-payload Master_Tag carrier
# ceremony.
# Every J-Link process is locked to probe 1050070698 and an explicit core.

readonly EXPECTED_SNR="1050070698"
readonly CARRIER_MARKER="master-tag-carrier-v2-fix10-relay5"
readonly PAYLOAD_MARKER="tag-fusion-link-relay5"
readonly EXPECTED_NET_SHA="c9003e16d921a4f8183bd7882e52fca3eb64da8f0a42285dfd239e76bfef429f"
readonly EXPECTED_APP_SHA="ba7d36e30b66502337309d32e411353511daaa6aafa196c638b765fd25772369"
readonly EXPECTED_PAYLOAD_SHA="00a73184f1c903f4fbf9471b134238518970f2e2514c9be7b5c75ffc19827ffa"
readonly EXPECTED_ZIP_SHA="5626e93c54cfcb5698a30d7166a2e547e4b88b6ca233fc03d2169e905d730ea0"
...
if [ "$#" -lt 1 ] || [ "$#" -gt 2 ]; then
	echo "Usage: $0 1050070698 [batchE3_20260729]" >&2
	exit 2
fi
...
ceremony_id="${2:-batchE_20260729}"
case "$ceremony_id" in
batchE_20260729 | batchE3_20260729) ;;
*)
...
build_dir="$repo_root/../builds/master-control-b120-m1-master-tag-lfrc-carrier-v2-fix10-relay5"
net_hex="$build_dir/hci_ipc/zephyr/merged_CPUNET.hex"
app_hex="$build_dir/zephyr/merged.hex"
app_elf="$build_dir/zephyr/zephyr.elf"
payload_bin="$repo_root/../builds/tag-fusion-link-relay5/tag/zephyr/zephyr.signed.bin"
payload_zip="$repo_root/../builds/tag-fusion-link-relay5/dfu_application.zip"
...
if [ "$ceremony_id" = "batchE3_20260729" ]; then
	log_root="$repo_root/../logs/batchE3_20260729/master_carrier_fix10_relay5_ceremony"
else
	log_root="$repo_root/../logs/batchE_bs065f_20260729/master_carrier_fix10_relay5_ceremony"
fi
...
grep -Fq "\"signed_bin_sha256\": \"$EXPECTED_PAYLOAD_SHA\"" "$payload_manifest" || {
	echo "[error] relay5 signed-bin SHA missing from payload manifest" >&2
	exit 11
}
grep -Fq "\"dfu_zip_sha256\": \"$EXPECTED_ZIP_SHA\"" "$payload_manifest" || {
	echo "[error] relay5 ZIP SHA missing from payload manifest" >&2
	exit 12
}
...
echo "[ok] fix10-relay5 dual-core ceremony programmed and verified"
