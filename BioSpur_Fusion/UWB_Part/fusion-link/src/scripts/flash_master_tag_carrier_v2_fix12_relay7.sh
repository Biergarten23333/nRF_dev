...
# Destructive, one-shot-per-authorized-batch relay7-payload Master_Tag carrier
# ceremony.
# Every J-Link process is locked to probe 1050070698 and an explicit core.

readonly EXPECTED_SNR="1050070698"
readonly CARRIER_MARKER="master-tag-carrier-v2-fix12-relay7"
readonly PAYLOAD_MARKER="tag-fusion-link-relay7"
readonly EXPECTED_NET_SHA="c9003e16d921a4f8183bd7882e52fca3eb64da8f0a42285dfd239e76bfef429f"
readonly EXPECTED_APP_SHA="65407cd882c88870a4c04df33d4f68979bfac8819dc2286fbac6041cb29c04c6"
readonly EXPECTED_PAYLOAD_SHA="a861274c159941f47e802ba5932f1e9769e3bbcc2bbb4ddcad8ca39fa8881189"
readonly EXPECTED_ZIP_SHA="9b5e221b7a4cd9940ac0dfd7d9a29cfe373b37d01164239a80add9bfb5084779"
readonly EXPECTED_ARCHIVE_SHA="93c95dbe975a3a256fe630e4eb53a0b6ed23dcd3b1a67d163e6aa8faacf82d37"
readonly EXPECTED_FIX10_APP_SHA="ba7d36e30b66502337309d32e411353511daaa6aafa196c638b765fd25772369"
readonly EXPECTED_FIX10_NET_SHA="c9003e16d921a4f8183bd7882e52fca3eb64da8f0a42285dfd239e76bfef429f"
...
if [ "$#" -lt 1 ] || [ "$#" -gt 2 ]; then
	echo "Usage: $0 1050070698 [night_20260730_morning]" >&2
	exit 2
fi
...
ceremony_id="${2:-night_20260730_morning}"
case "$ceremony_id" in
night_20260730_morning) ;;
*)
...
build_dir="$repo_root/../builds/master-control-b120-m1-master-tag-lfrc-carrier-v2-fix12-relay7"
net_hex="$build_dir/hci_ipc/zephyr/merged_CPUNET.hex"
app_hex="$build_dir/zephyr/merged.hex"
app_elf="$build_dir/zephyr/zephyr.elf"
payload_bin="$repo_root/../builds/tag-fusion-link-relay7/tag/zephyr/zephyr.signed.bin"
payload_zip="$repo_root/../builds/tag-fusion-link-relay7/dfu_application.zip"
...
fix10_build="$repo_root/../builds/master-control-b120-m1-master-tag-lfrc-carrier-v2-fix10-relay5"
fix10_app_hex="$fix10_build/zephyr/merged.hex"
fix10_net_hex="$fix10_build/hci_ipc/zephyr/merged_CPUNET.hex"
log_root="$repo_root/../logs/night_20260730/morning/carrier_fix12_relay7/ceremony"
...
for artifact in \
	...
	"$archive_tar" "$archive_sums" "$fix10_app_hex" "$fix10_net_hex"; do
	[ -f "$artifact" ] || {
...
actual_fix10_app_sha="$(sha256sum "$fix10_app_hex" | awk '{print $1}')"
actual_fix10_net_sha="$(sha256sum "$fix10_net_hex" | awk '{print $1}')"
...
[ "$actual_fix10_app_sha" = "$EXPECTED_FIX10_APP_SHA" ] || {
	echo "[error] resident fix10-relay5 CPUAPP restore SHA mismatch" >&2
	exit 9
}
[ "$actual_fix10_net_sha" = "$EXPECTED_FIX10_NET_SHA" ] || {
	echo "[error] resident fix10-relay5 CPUNET restore SHA mismatch" >&2
	exit 9
}
grep -Fq "\"fw_marker\": \"$PAYLOAD_MARKER\"" "$payload_manifest" || {
	echo "[error] relay5 marker missing from payload manifest" >&2
	exit 10
}
grep -Fq "\"signed_bin_sha256\": \"$EXPECTED_PAYLOAD_SHA\"" "$payload_manifest" || {
	echo "[error] relay7 signed-bin SHA missing from payload manifest" >&2
	exit 11
}
grep -Fq "\"dfu_zip_sha256\": \"$EXPECTED_ZIP_SHA\"" "$payload_manifest" || {
	echo "[error] relay7 ZIP SHA missing from payload manifest" >&2
	exit 12
}
...

{
	...
	echo "fix10_restore_cpuapp_sha256=$actual_fix10_app_sha"
	echo "fix10_restore_cpunet_sha256=$actual_fix10_net_sha"
...
echo "[ok] fix12-relay7 dual-core ceremony programmed and verified"
