...
struct master_peer {
	...
	char tdma_cfg_tx[200];
	bool tdma_cfg_tx_pending;
...
static void peer_clear(unsigned int idx, bool unref_conn)
{
	...
	peers[idx].tdma_cfg_tx_pending = false;
				      ...
				      uint8_t positioning_mode)
{
	...
	if (peer->tdma_cfg_tx_pending) {
		printk("CFG send deferred[%d]: previous write still pending\n",
		       peer_index_from_nus(&peer->nus_client));
		return -EALREADY;
	}

	if (MASTER_TDMA_SLOT_ACTIVE_US != 0U) {
		snprintk(peer->tdma_cfg_tx, sizeof(peer->tdma_cfg_tx),
			 "CFG TAG=%u SLOT=%u COUNT=%u MASK=0x%04X PERIOD=%u ACTIVE=%u ACTIVE_US=%u EPOCH=%lu SUPERFRAME_BASE=%lu GEN=%u PMODE=%u AMODE=%u",
			 ...
			 (unsigned long)superframe_base,
			 (unsigned int)generation,
			 (unsigned int)positioning_mode,
			 0U);
	} else {
		snprintk(peer->tdma_cfg_tx, sizeof(peer->tdma_cfg_tx),
			 "CFG TAG=%u SLOT=%u COUNT=%u MASK=0x%04X PERIOD=%u ACTIVE=%u EPOCH=%lu SUPERFRAME_BASE=%lu GEN=%u PMODE=%u AMODE=%u",
			 (unsigned int)logical_tag_id,
			 (unsigned int)slot_index,
			 (unsigned int)slot_count,
			 (unsigned int)slot_mask,
	...
	/*
	 * bt_nus_client_send() retains the caller's data pointer until its
	 * asynchronous GATT write callback.  Keep CFG storage in the peer
	 * instead of on this function's stack so the command tail (notably
	 * SUPERFRAME_BASE) cannot be overwritten before transmission.
	 */
	peer->tdma_cfg_tx_pending = true;
	err = bt_nus_client_send(&peer->nus_client,
				 (const uint8_t *)peer->tdma_cfg_tx,
				 strlen(peer->tdma_cfg_tx));
	if (err) {
		peer->tdma_cfg_tx_pending = false;
	...
	printk("CFG assigned[%d]: bs=BS%04X tag=%u slot=%u/%u mask=0x%04X period=%u active=%u active_us=%u superframe_base=%lu gen=%u pmode=%u\n",
	       peer_index_from_nus(&peer->nus_client),
	       (unsigned int)peer->bs_code,
	       (unsigned int)logical_tag_id,
	       (unsigned int)slot_index,
...
assign_slots:
	tdma_generation++;
	epoch_deadline_ms =
		(uint64_t)k_uptime_get() + MASTER_TDMA_EPOCH_LEAD_MS;
	superframe_base = (uint32_t)(epoch_deadline_ms / 100U);
	for (size_t i = 0U; i < ready_count; ++i) {
		...
		epoch_delay_ms = (epoch_deadline_ms > now_ms) ?
			(uint32_t)(epoch_deadline_ms - now_ms) : 0U;
			  ...
			  const uint8_t *const data, uint16_t len)
{
	...
	if (idx >= 0 && data == (const uint8_t *)peers[idx].tdma_cfg_tx) {
		peers[idx].tdma_cfg_tx_pending = false;
	}
...
static void scan_recv(const struct bt_le_scan_recv_info *info, struct net_buf_simple *buf)
{
	...
	if (!bs_code_valid && runtime_target_kind == MASTER_TARGET_TAG &&
		...
		bs_code_valid = true;
		tag_id = (uint8_t)(bs_code & 0xffU);
		tag_id_valid = true;
	}

	/*
	 * Candidate logging is an observation instrument, not an acceptance
	 * result.  Keep it before every role/allow-list filter so an empty scan
	 * log cannot falsely make a filtered peer look radio-silent.
	 */
	if (peer_index_from_addr(info->addr) >= 0) {
		master_led_flow_pulse();
		return;
	}
	if (bs_code_valid || name_match || anchor_name_match || nus_match ||
	    dfu_match || token_match) {
		scan_log_candidate(info, buf, name_match, anchor_name_match,
				  nus_match, dfu_match, token_match, uuid_hex,
				  uuid_match, bs_code, bs_code_valid);
	}
	...
	if (runtime_target_kind == MASTER_TARGET_TAG &&
		...
		printk("RECV candidate rejected: %s bs=%s not in boot tag allow-list=%s\n",
		       addr, bs_name, APP_MASTER_BOOT_TAG_ALLOWLIST);
		return;
	}
