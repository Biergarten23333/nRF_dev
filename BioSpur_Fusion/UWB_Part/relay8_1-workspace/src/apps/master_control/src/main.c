...
#endif
#ifndef APP_MASTER_CARRIER_MARKER
#define APP_MASTER_CARRIER_MARKER "master-carrier-unversioned"
#endif
...
static uint32_t app_reset_reason;
...
{
	printk("Commands: status | netreset | mode recv | mode ota | mode autopos | scan | conn | initiate\n");
...
static void control_handle_uart_command(const char *line)
{
	...
	if (strcmp(cmd, "netreset") == 0) {
		/*
		 * Do not call bt_disable() here: its HCI RESET handshake depends on
		 * the very CPUNET that this command must recover. Force CPUNET fully
		 * off and back on (the HAL includes nRF53 erratum 161), then reboot
		 * CPUAPP so IPC and the Bluetooth host start from one clean epoch.
		 */
		printk("NETRESET begin mechanism=forceoff+sysresetreq\n");
		nrf_reset_network_force_off(NRF_RESET, true);
		k_sleep(K_MSEC(20));
		nrf_reset_network_force_off(NRF_RESET, false);
		printk("NETRESET forceoff released; rebooting CPUAPP\n");
		k_sleep(K_MSEC(100));
		sys_reboot(SYS_REBOOT_COLD);
		CODE_UNREACHABLE;
	}
...
int main(void)
{
	...
	app_reset_reason = nrf_reset_resetreas_get(NRF_RESET);
	nrf_reset_resetreas_clear(NRF_RESET, app_reset_reason);
	...
	printk("APP_BOOT carrier=%s resetreas=0x%08x\n",
	       APP_MASTER_CARRIER_MARKER, app_reset_reason);
