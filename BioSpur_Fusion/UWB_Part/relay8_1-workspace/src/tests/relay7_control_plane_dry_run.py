#!/usr/bin/env python3
"""Offline round-trip bound and parser dry run for relay7 commands."""
...
# Canonical Path-R composition. Optional legacy CFG fields are omitted; the
# parser retains them, but a host composer must reject an all-option line that
# exceeds the 200-byte outer control bound.
raw_cfg = (
    "CFG TAG=15 SLOT=254 COUNT=255 PERIOD=65535 ACTIVE=65535 "
    "EPOCH=4294967295 BEACON_SYNC=1 BEACON_WIN_N=10 "
...
cfg_ok = (
    ...
    "BEACON_WIN_N=10 DW_ANCHOR=1 LIVE=1 RUN=1 STATE=RUNNING"
)
...
beacon_reply = (
    ...
    "dwmiss=4294967295 win=10"
)

for name, value, bound in (
    ("canonical raw CFG UART payload", raw_cfg, UART_PAYLOAD_MAX),
    ("canonical wrapped CFG control line", host_cfg, CONTROL_LINE_MAX),
    ("CFG_OK worst-case reverse reply", cfg_ok, UART_PAYLOAD_MAX),
    ...
    ("BEACON_STATUS worst-case reverse reply", beacon_reply, UART_PAYLOAD_MAX),
):
...
all_optional_cfg = (
    ...
    "BEACON_WIN_N=10 DW_ANCHOR=1 RUN=1 PMODE=3"
)
assert len(all_optional_cfg.encode("ascii")) <= UART_PAYLOAD_MAX
assert len(("BSF3C79 TAG RAW " + all_optional_cfg).encode("ascii")) > CONTROL_LINE_MAX
print("all-option CFG: host composer MUST reject before send PASS")

cfg_pattern = re.compile(
    r"^CFG TAG=(?P<tag>\d+) SLOT=(?P<slot>\d+) COUNT=(?P<count>\d+) "
    r"PERIOD=(?P<period>\d+) ACTIVE=(?P<active>\d+) EPOCH=(?P<epoch>\d+) "
    r"BEACON_SYNC=(?P<sync>[01]) BEACON_WIN_N=(?P<win>10|[1-9]) "
...
assert "beacon_win_n" not in tag_source[settings_start:settings_end]
...
assert "record.beacon_win_n" not in tag_source[store_start:store_end]
...
print("target parser/source, volatile storage, and reverse bounds: PASS")
