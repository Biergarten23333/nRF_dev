				  ...
				  uint8_t generation)
{
	...
	schedule->sync_local_sub_ms_us = 0U;
	schedule->epoch_valid = true;
	schedule->generation = generation;
}

void uwb_tdma_sync_schedule_local_epoch_us(
	struct uwb_tdma_schedule *schedule, uint64_t local_epoch_us)
{
	if (schedule == NULL) {
		return;
	}
	...
	schedule->epoch_ms = 0U;
	schedule->sync_local_ms = (uint32_t)(local_epoch_us / 1000U);
	schedule->sync_local_sub_ms_us =
		(uint16_t)(local_epoch_us % 1000U);
	schedule->epoch_valid = true;
}
...
uint32_t uwb_tdma_schedule_time_until_next_slot_ms(
	const struct uwb_tdma_schedule *schedule)
{
	uint32_t cycle_ms;
	uint32_t phase_ms;
	...
	for (uint8_t slot = 0U; slot < schedule->slot_count; ++slot) {
		...
		slot_start_ms =
			(uint32_t)slot * (uint32_t)schedule->slot_period_ms;
		delta_ms = slot_start_ms > phase_ms ?
			slot_start_ms - phase_ms :
			cycle_ms - phase_ms + slot_start_ms;
		if (delta_ms < best_ms) {
			best_ms = delta_ms;
		}
	}
...
uint32_t uwb_tdma_schedule_time_until_next_slot_us(
	const struct uwb_tdma_schedule *schedule)
{
	...
	if (!uwb_tdma_schedule_is_valid(schedule) ||
	    !schedule->epoch_valid) {
		return 0U;
	}

	local_now_us = k_ticks_to_us_floor64(k_uptime_ticks());
	epoch_us = (uint64_t)schedule->sync_local_ms * 1000U +
		   schedule->sync_local_sub_ms_us;
	cycle_us = (uint32_t)schedule->slot_count *
		   (uint32_t)schedule->slot_period_ms * 1000U;
	if (local_now_us < epoch_us) {
		uint32_t first_slot_us = cycle_us;

		slot_mask = uwb_tdma_effective_slot_mask(schedule);
		for (uint8_t slot = 0U; slot < schedule->slot_count; ++slot) {
			if ((slot_mask & (uint16_t)(1U << slot)) != 0U) {
				first_slot_us = (uint32_t)slot *
					(uint32_t)schedule->slot_period_ms *
					1000U;
				break;
			}
		...
		return (uint32_t)MIN(
			epoch_us - local_now_us + first_slot_us,
			UINT32_MAX);
	}
	phase_us = (uint32_t)((local_now_us - epoch_us) % cycle_us);
	slot_mask = uwb_tdma_effective_slot_mask(schedule);
	best_us = cycle_us;
	...
	for (uint8_t slot = 0U; slot < schedule->slot_count; ++slot) {
		...
		slot_start_us = (uint32_t)slot *
				(uint32_t)schedule->slot_period_ms * 1000U;
		delta_us = slot_start_us > phase_us ?
			slot_start_us - phase_us :
			cycle_us - phase_us + slot_start_us;
		if (delta_us < best_us) {
			best_us = delta_us;
		}
	}
