#!/usr/bin/env python3
"""STEP 3 v2 — multipath extraction from templates_v2 (corrected FP_INDEX
sub-tap alignment). Identical algorithm to step3_multipath.py; only the input
template dir (templates_v2) and output dir (multipath_v2) change, so the
reflector tables reflect the tighter v2 alignment (higher DP-SNR, sharper main
lobe). Reflector = tap cluster >5*sigma outside the direct-path main lobe."""
import os, csv, json, glob
import numpy as np

HERE = os.path.dirname(os.path.abspath(__file__))
TPL = os.path.join(HERE, "templates_v2")
OUT = os.path.join(HERE, "multipath_v2")
...
for apath in sorted(glob.glob(os.path.join(TPL, "*_A.npy"))):
    ...
    over = mag > thresh
    over[REF_TAP - FP_GUARD: mainlobe_end + 1] = False
    refls = []
    i = 0
    n = over.size
    while i < n:
    ...
    blind_mm = round(float((mainlobe_end - REF_TAP) * TAP_NS * C), 1)
    ...
    print(f"[Step3v2] {base}: dp_peak@{dp_peak} ML_end@{mainlobe_end} blind<{blind_mm}mm | "
...
print(f"\n[Step3v2] {len(rows)} channels: reflectors/channel "
      f"mean={np.mean(nref):.1f} median={np.median(nref):.0f} max={max(nref)}")
print(f"[Step3v2] wrote {OUT}/all_reflectors_summary.csv")
