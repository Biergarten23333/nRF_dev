...
def default_port_specs_from_env():
    raw_value = os.environ.get("BIOSPUR_CIR_FULL_USB_PORTS", "").strip()
    if not raw_value:
        return []
    specs = []
...
class CaptureState:
    ...
    def finish_frame(self, port_label: str, done: dict):
        ...
        with self.lock:
            ...
            self.meta_f.flush()
            preview_every = max(0, int(getattr(self.args, "preview_every", 1)))
            if preview_every > 0 and (frame_index == 1 or frame_index % preview_every == 0):
                wave = downsample_accumulator(buf)
                print(
                    "[FULL-CIR] frame={frame} port={port} rx={rx} src={src} "
...
def main():
    ...
    ap.add_argument("--port", action="append", default=[], help="USB CDC port, optionally LABEL=/dev/...")
    ap.add_argument("--seconds", type=float, required=True)
    ap.add_argument("--capture-root", required=True)
    ap.add_argument("--target", default="BSF66F")
    ap.add_argument("--baud", type=int, default=115200)
    ap.add_argument("--preview-every", type=int, default=0, help="Print one FULL-CIR preview line every N frames; 0 disables preview lines")
    ...
    if not args.port:
        args.port = default_port_specs_from_env()
        if args.port:
            print("[CIRRAW] using ports from BIOSPUR_CIR_FULL_USB_PORTS", flush=True)
        else:
            ap.error("the following arguments are required: --port (or set BIOSPUR_CIR_FULL_USB_PORTS)")
