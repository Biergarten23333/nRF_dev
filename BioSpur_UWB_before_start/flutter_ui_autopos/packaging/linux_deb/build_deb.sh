...
if [[ ! -x "${BUILD_DIR}/flutter_ui_autopos" ]]; then
...
#!/usr/bin/env bash
exec /opt/biospur-autopos/flutter_ui_autopos "$@"
EOF
chmod 0755 "${PKG_ROOT}/usr/bin/${APP_NAME}"

cat > "${PKG_ROOT}/usr/share/applications/com.biospur.autopos.desktop" <<EOF
[Desktop Entry]
Type=Application
Name=${DISPLAY_NAME}
...
StartupNotify=true
StartupWMClass=com.biospur.autopos
EOF
