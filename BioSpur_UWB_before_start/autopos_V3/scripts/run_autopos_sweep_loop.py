...
def quarantine_tag_for_sweep(
    ...
    if f"ota_target name rc=0 value={target_name.lower()}" not in target_text and \
       f"ota_target name rc=0 value={target_name}" not in target_text:
        emit(logf, f"PRECHECK FAIL: ota_target name {target_name} not acknowledged\n", live_output, verbose)
        return ser, False

    ready_ok = ("DISC complete[0]" in target_text and "CFG_OK" in target_text)
    if not ready_ok:
        ser, ready_ok, _ = wait_for_patterns(
            ser,
            logf,
            port,
            ["DISC complete[0]", "CFG_OK"],
            10.0,
            live_output,
            verbose,
        )
    ...
    try:
        ...
        with open(log_path, "w", buffering=1) as logf:
            ...
            if quiet_tag_name:
                ...
                if not quiet_ok:
                    result["error"] = f"tag_quiet_failed:{quiet_tag_name}"
                    emit(logf, "PRECHECK FAIL: tag quarantine not reached\n", live_output, verbose)
