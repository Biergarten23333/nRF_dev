...
APP_VERSION="${2:-1}"
ZIP_OUT="${3:-biospur_dongle_scanner.zip}"
...
if [[ -z "${NRFUTIL_BIN}" ]]; then
  if [[ -x "${HOME}/.cache/biospur_nrfutil_venv/bin/nrfutil" ]]; then
    ...
    echo "[err] nrfutil not found in PATH and scripts/nrfutil is unavailable" >&2
    exit 1
  fi
fi
...
if [[ ! -f "${HEX_PATH}" ]]; then
  ...
  echo "[hint] run: scripts/build_52840_dongle_scanner.sh ${BUILD_DIR}" >&2
  exit 1
fi
