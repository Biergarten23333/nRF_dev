# BUILD.md — rebuild the five freeze binaries from this source snapshot

Source snapshot of the four-piece firmware + listener at git tag **`freeze-clean-20260716`**
(commit `8b68ee0a`). Copied from `SS-TWR/alt-SS-TWR/broadcast/` — this dir **mirrors that
tree**, and the build scripts compute `repo_root="$(dirname "$0")/.."`, so **this snapshot is
a drop-in buildable tree**: run the scripts from here and they resolve `apps/*`, `UWB_listener`,
`configs/`, `src/`, `drivers/`, `apps/master_ota/generated/` all within this folder.
...
## Build each piece (run from THIS directory)
```bash
