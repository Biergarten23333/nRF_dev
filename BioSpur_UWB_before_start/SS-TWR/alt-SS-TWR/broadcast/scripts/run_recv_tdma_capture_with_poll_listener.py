...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run RECV/TDMA tag capture while recording generic co-located listener LPD rows."
    )
    parser.add_argument(
        "--listener-port",
        required=True,
        help="New generic poll listener serial port. No default is provided to avoid the legacy 760185886 listener.",
    )
    parser.add_argument(
        "--out-dir",
        default=f"logs/recv_tdma_with_poll_listener_{time.strftime('%Y%m%d_%H%M%S')}",
    ...
    parser.add_argument(
        ...
        type=float,
        default=360.0,
        help="Keep listener running this many seconds longer than RECV capture to cover preflight and recovery.",
    )
    parser.add_argument(
        "capture_args",
...
def main() -> int:
    ...
    listener_cmd = [
        sys.executable,
        "scripts/capture_uwb_poll_listener.py",
        "--port",
        args.listener_port,
        "--duration",
