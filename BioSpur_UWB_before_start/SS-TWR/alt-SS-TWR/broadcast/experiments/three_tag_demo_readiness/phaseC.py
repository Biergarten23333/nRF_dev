#!/usr/bin/env python3
"""Phase C: distinct-per-tag-slot TDMA (tdma auto) — N cold-start repeatability + persistence.
Safe entry: reboot -> settle -> tdma roster <tag> motion x3 -> tdma auto 1 -> verify LIVE=1 -> measure.
"""
...
TAGS=["BS9336","BS955A","BSCCF4"]; GOOD_ALL=0.90; SETTLE=16; MEAS=30; N=10; HOLD_S=780; HOLD_BIN=60
res=json.load(open(os.path.join(OUT,"results.json")))
res["phase_C"]={"entry":"reboot -> tdma roster <tag> motion x3 -> tdma auto 1 -> verify","cold_starts":[]}
...
def apply_distinct():
    ...
    live=out.count("LIVE=1"); confirmed=out.count("CFG_OK TAG=")
    return {"live_ok":live,"cfg_ok":confirmed}
def measure(secs):
    ...
    m.close()
    per=defaultdict(lambda:{"n":0,"ge7":0,"ge8":0,"raw":0})
...
log(f"=== PHASE C: distinct-slot TDMA, N={N} cold starts ===")
goods=0
for i in range(1,N+1):
    ...
    if gd: goods+=1
    res["phase_C"]["cold_starts"].append({"i":i,"apply":ap,"tags":m,"geiger":g,"good":gd}); save()
    log(f"  [C{i}] "+" ".join(f"{t}={m[t]['ge7']:.0%}" for t in TAGS)+f" | apply(live={ap['live_ok']},cfg={ap['cfg_ok']}) pollers={g} GOOD={gd}")
res["phase_C"]["good_count"]=goods; save()
log(f"  PHASE C: {goods}/{N} good cold starts")
# persistence: apply distinct-slot ONCE, hold the SAME session (proper persistence test)
log(f"  PHASE C persistence hold ({HOLD_S}s) on the distinct-slot session:")
cmd("cmd_all REBOOT",2.5); time.sleep(SETTLE); apply_distinct(); time.sleep(4)
bins=[]; t0=time.time()
while time.time()-t0<HOLD_S:
    b=measure(HOLD_BIN); row={t:b[t]["ge7"] for t in TAGS}; row["ok"]=good(b); row["t"]=int(time.time()-t0)
    bins.append(row); res["phase_C"]["hold"]=bins; save()
    log(f"    t={row['t']}s "+" ".join(f"{t}={b[t]['ge7']:.0%}" for t in TAGS)+f" ok={row['ok']}")
log("=== DONE Phase C ==="); save()
