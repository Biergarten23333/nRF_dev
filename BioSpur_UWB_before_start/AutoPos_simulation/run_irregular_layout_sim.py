...
def generate_irregular_layout(rng: np.random.Generator, layout_id: int) -> tuple[np.ndarray, dict[str, Any]]:
    for attempt in range(2000):
        width = rng.uniform(1800.0, 4700.0)
        depth = rng.uniform(1600.0, 4700.0)
        base = np.array(
            [
                [0.0, 0.0, 0.0],
                [width, 0.0, 0.0],
        ...
        xyz = gauge_align_local(normalize_extent_mm(xyz))

        span = np.max(xyz, axis=0) - np.min(xyz, axis=0)
        if np.max(span) > 5000.0 + 1e-6:
            continue
...
def write_layout(path: Path, xyz: np.ndarray, meta: dict[str, Any], solver: str = "truth") -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    data = {
        "solver": solver,
        "anchors": [
            {"id": i, "label": ANCHORS[i], "x_mm": float(xyz[i, 0]), "y_mm": float(xyz[i, 1]), "z_mm": float(xyz[i, 2]), "d_anchor_mm": 0.0}
            for i in range(8)
        ],
        "meta": meta,
    }
...
def solve_one_layout(task: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    xyz_true, meta = generate_irregular_layout(rng, layout_id)
...
def main() -> int:
    ...
    tasks = [
        {"layout_id": i, "out_root": str(out_root), "sets": args.sets, "seed": args.seed, "noise_bank": serializable_noise}
        for i in range(args.layouts)
    ]
    all_rows: list[dict[str, Any]] = []
    if args.workers <= 1:
        for task in tasks:
            print(f"[sim] layout {task['layout_id'] + 1}/{args.layouts}", flush=True)
    ...
    all_rows.sort(key=lambda r: (int(r["layout_id"]), SOLVER_NAMES.index(str(r["solver"]))))
    write_csv(out_root / "summary.csv", all_rows)
    agg = aggregate_by_solver(all_rows)
    write_csv(out_root / "summary_by_solver.csv", agg)
    save_figures(out_root, all_rows)
