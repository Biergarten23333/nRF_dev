...
APP_NAME="biospur-cir"
DISPLAY_NAME="BioSpur CIR Realtime Monitor"
...
if [[ ! -x "${BUILD_DIR}/flutter_ui_cir" ]]; then
  echo "[deb] missing release bundle; run: ../.tooling/flutter/bin/flutter build linux --release" >&2
  exit 1
fi
...
#!/usr/bin/env bash
exec /opt/biospur-cir/flutter_ui_cir "$@"
EOF
chmod 0755 "${PKG_ROOT}/usr/bin/${APP_NAME}"

cat > "${PKG_ROOT}/usr/share/applications/com.biospur.cir.desktop" <<EOF
[Desktop Entry]
Type=Application
Name=${DISPLAY_NAME}
Comment=BioSpur CIR realtime monitor
Exec=${APP_NAME}
Icon=${APP_NAME}
Terminal=false
Categories=Science;Utility;
StartupNotify=true
StartupWMClass=com.biospur.cir
EOF
...
Description: BioSpur CIR Realtime Monitor
 Desktop UI for UWB CRX/CIR feature monitoring.
EOF
