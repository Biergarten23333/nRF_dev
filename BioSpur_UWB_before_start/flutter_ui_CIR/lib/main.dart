...
String anchorLabelForId(int id) =>
    id >= 0 && id < anchors.length ? anchors[id] : 'A$id';
const masterAnchorControlPort =
...
String get settingsPath {
  ...
  return '$home/.config/biospur-cir/settings.json';
}
...
const appBuildStamp = '2026-06-02 17:25 CEST';
const appBuildNote = 'USB data path for Tag and Anchor CIR';

class CirFieldApp extends StatelessWidget {
  const CirFieldApp({super.key});

  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      debugShowCheckedModeBanner: false,
      title: 'BioSpur CIR Realtime Monitor',
      theme: ThemeData(
        useMaterial3: true,
              ...
              Material(
                ...
                child: TabBar(
                  ...
                  tabs: [
                    ...
                    const Tab(icon: Icon(Icons.sensors_outlined), text: 'CIR'),
                  ],
                ),
              ),
              Expanded(
                child: TabBarView(
                  ...
                  children: [
                    ...
                    CirTab(runner: _runner, ports: _ports),
                  ],
                ),
              ),
...
class CirTab extends StatefulWidget {
  const CirTab({super.key, required this.runner, required this.ports});

  final ScriptRunner runner;
  final PortSnapshot ports;

  @override
  State<CirTab> createState() => _CirTabState();
}

class _CirTabState extends State<CirTab>
    with AutomaticKeepAliveClientMixin<CirTab> {
  final TextEditingController _target = TextEditingController(text: 'BSF66F');
  final TextEditingController _duration = TextEditingController(text: '120');
  bool _fullRawUsb = true;
  ...
  Future<void> _startMonitor() async {
    final ports = await PortReader.read();
    if (!mounted) return;
    if (_fullRawUsb && ports.cirUsbCount == 0) {
      await showBioSpurNotice(
        context,
        title: 'CIR USB data ports not connected',
        message:
            'Full CIR monitor reads Tag and Anchor J-Link CDC ports directly. Connect at least one CIR USB data port, then press Start Monitor again.',
      );
      return;
    }
    if (!_fullRawUsb && !ports.masterTag) {
      await showBioSpurNotice(
        context,
        title: 'Master Tag not connected',
        message:
            'CIR monitor requires the Master_Tag CDC port. Connect the Master Tag board, then press Connect again.',
      );
      return;
    }
    final target = _target.text.trim().isEmpty ? 'BSF66F' : _target.text.trim();
    final seconds = int.tryParse(_duration.text.trim()) ?? 120;
    if (seconds <= 0) {
      await showBioSpurNotice(
        context,
        title: 'Invalid duration',
        message: 'Set monitor seconds to a positive number.',
      );
      return;
    }
    final command = _fullRawUsb
        ? _cirRawFullUsbCommand(target: target, seconds: seconds)
        : _cirMonitorCommand(target: target, seconds: seconds);
    await widget.runner.start(
      _fullRawUsb ? 'CIR RAW $target' : 'CIR $target',
      command,
    );
  }

  String _cirRawFullUsbCommand({required String target, required int seconds}) {
    const script = '$repoRoot/flutter_ui_CIR/scripts/cir_full_usb_capture.py';
    final portArgs = cirUsbPorts.entries
        .map((entry) => '  --port ${shellQuote('${entry.key}=${entry.value}')} \\')
        .join('\n');
    return '''
set -euo pipefail
python3 ${shellQuote(script)} \\
$portArgs
  --seconds ${shellQuote(seconds.toString())} \\
  --capture-root ${shellQuote(capturesRoot)} \\
  --target ${shellQuote(target)} \\
  --control-port ${shellQuote(masterAnchorControlPort)}
''';
  }

  String _cirMonitorCommand({required String target, required int seconds}) {
    const port =
        '/dev/serial/by-id/usb-Master_Tag_BioSpur_BLE_Control_6918E0384172A49F-if00';
    return '''
set -euo pipefail
export TARGET=${shellQuote(target)}
export MONITOR_SECONDS=${shellQuote(seconds.toString())}
export PORT=${shellQuote(port)}
export CAPTURE_ROOT=${shellQuote(capturesRoot)}
python3 -u - <<'PY'
import csv
import os
import re
...
duration = float(os.environ["MONITOR_SECONDS"])
port = os.environ["PORT"]
capture_root = Path(os.environ["CAPTURE_ROOT"])
safe_target = re.sub(r"[^A-Za-z0-9_.-]+", "_", target).strip("_") or "tag"
stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
capture_dir = capture_root / f"CIR_{safe_target}_{stamp}"
raw_path = capture_dir / "crx_raw.log"
csv_path = capture_dir / "crx_samples.csv"
...
def anchor_label(anchor_id):
    labels = "ABCDEFGH"
    return labels[anchor_id] if 0 <= anchor_id < len(labels) else f"A{anchor_id}"
...
def parse_crx(line):
    ...
    if len(parts) < 15 or parts[0] != "CRX" or parts[1] != "1":
        return None
    try:
        sweep = int(parts[2])
        ...
        fp_amp1 = int(parts[8])
        fp_amp2 = int(parts[9])
        fp_amp3 = int(parts[10])
        peak = int(parts[11])
        pream = int(parts[12])
        std_noise = int(parts[13])
        max_noise = int(parts[14])
    except ValueError:
        return None
    return {
        ...
        "sweep": sweep,
        "anchor_id": anchor_id,
        "anchor": anchor_label(anchor_id),
        "raw_distance_mm": raw_mm,
        "rx_timestamp": rx_ts,
        "carrier_integrator": ci,
        "first_path": fp,
        "first_path_amp1": fp_amp1,
        "first_path_amp2": fp_amp2,
        "first_path_amp3": fp_amp3,
        "fp_amp_sum": fp_amp1 + fp_amp2 + fp_amp3,
        "max_growth_cir": peak,
        "rx_pream_count": pream,
        "std_noise": std_noise,
        "max_noise": max_noise,
        "raw_line": line,
    }
...
def read_for(ser, seconds):
    ...
    while time.monotonic() < end and not stop_requested:
        ...
        if not raw:
            continue
        print(raw.decode("utf-8", "replace").rstrip(), flush=True)

def append_session_note(sample_count):
    ...
    with notes_path.open("a", encoding="utf-8", newline="") as f:
        ...
        writer.writerow([
            ...
            f"CIR_{safe_target}_{stamp}",
            "cir",
            str(capture_dir),
            f"target={target}; duration_s={duration:.0f}; samples={sample_count}",
        ])

print(f"[CIR] port={port} target={target} duration_s={duration:.0f}", flush=True)
capture_dir.mkdir(parents=True, exist_ok=True)
print(f"[CIR] save_dir={capture_dir}", flush=True)
with serial.Serial(port, 115200, timeout=0.2, write_timeout=2, exclusive=True, dsrdtr=False, rtscts=False) as ser:
    ser.reset_input_buffer()
    for cmd, wait_s in [
        ("status", 1.0),
        ("ota_target uuid -", 0.4),
        (f"ota_target name {target}", 0.4),
        ...
        ser.flush()
        read_for(ser, wait_s)
    print("[CIR] capture start", flush=True)
    fieldnames = [
        "host_time_iso",
        "target",
        "sweep",
    ...
    with raw_path.open("a", encoding="utf-8") as raw_f, csv_path.open("w", encoding="utf-8", newline="") as csv_f:
        ...
        while time.monotonic() < end and not stop_requested:
            ...
            csv_f.flush()
            sample_count += 1
    append_session_note(sample_count)
    print(f"[CIR] capture done samples={sample_count}", flush=True)
    print(f"[CIR] saved_csv={csv_path}", flush=True)
    print(f"[CIR] saved_raw={raw_path}", flush=True)
PY
''';
  }
  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
              ...
              children: [
                ...
                Wrap(
                  ...
                  children: [
                    SegmentedButton<bool>(
                      segments: const [
                        ButtonSegment(
                          ...
                          label: Text('USB CIR data'),
                        ),
                        ButtonSegment(
                          value: false,
                          icon: Icon(Icons.sensors),
                          label: Text('BLE control'),
                        ),
                      ],
                      selected: {_fullRawUsb},
                      onSelectionChanged: widget.runner.isRunning
                          ? null
                          : (selection) {
                              setState(() {
                                _fullRawUsb = selection.first;
                              });
                            },
                    ),
                    SizedBox(
                    ...
                    FilledButton.icon(
                      onPressed: widget.runner.isRunning ? null : _startMonitor,
                      icon: const Icon(Icons.sensors),
                    ...
                    StatusPill(
                      label: '',
                      value: _fullRawUsb
                          ? (widget.ports.cirUsbCount > 0
                                ? 'USB data ${widget.ports.cirUsbCount}/9 online'
                                : 'USB data offline')
                          : (widget.ports.masterTag
                                ? 'Master_Tag online'
                                : 'Master_Tag offline'),
                      tone: _fullRawUsb
                          ? (widget.ports.cirUsbCount > 0
        ...
        const SizedBox(height: 14),
        if (_fullRawUsb) ...[
          CirRawWaveformCard(runner: widget.runner),
          const SizedBox(height: 14),
          CirRealtimeMonitorCard(runner: widget.runner),
        ] else ...[
          CirRealtimeMonitorCard(runner: widget.runner),
          const SizedBox(height: 14),
          CirRawWaveformCard(runner: widget.runner),
...
class CirRealtimeMonitorCard extends StatelessWidget {
  const CirRealtimeMonitorCard({super.key, required this.runner});
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      animation: runner,
      builder: (context, _) {
        final samples = CirCrxSample.fromLines(runner.logTail);
        final latestByAnchor = <int, CirCrxSample>{};
        for (final sample in samples) {
          latestByAnchor.putIfAbsent(sample.anchorId, () => sample);
        }
        final latest = samples.take(24).toList();
        return Card(
          child: Padding(
            padding: const EdgeInsets.all(14),
            child: Column(
              ...
              children: [
                ...
                GridView.builder(
                  ...
                  itemBuilder: (context, index) {
                    return CirAnchorTile(
                      sample: latestByAnchor[index],
                      index: index,
                    );
                  },
                ),
                ...
                Container(
                  ...
                  child: latest.isEmpty
                      ...
                      : SingleChildScrollView(
                          ...
                          child: DataTable(
                            columns: const [
                              DataColumn(label: Text('Sweep')),
                              DataColumn(label: Text('Anchor')),
                              DataColumn(label: Text('TWR raw')),
                              DataColumn(label: Text('FP amp')),
                              DataColumn(label: Text('Peak')),
                              DataColumn(label: Text('Noise')),
                              DataColumn(label: Text('Quality')),
                            ],
                            rows: latest.map((sample) {
                              return DataRow(
                                cells: [
                                  DataCell(Text('${sample.sweep}')),
                                  DataCell(
                                    Text(anchorLabelForId(sample.anchorId)),
                                  ),
                                  DataCell(Text('${sample.rawDistanceMm}')),
                                  DataCell(Text('${sample.fpAmp}')),
                                  DataCell(Text('${sample.maxGrowthCir}')),
                                  DataCell(Text('${sample.stdNoise}')),
                                  DataCell(Text(sample.qualityLabel)),
                                ],
                              );
                            }).toList(),
                          ),
...
class CirRawWaveformCard extends StatelessWidget {
  const CirRawWaveformCard({super.key, required this.runner});
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      animation: runner,
      builder: (context, _) {
        final samples = CirPlotSample.fromLines(runner.logTail);
        final latestByAnchor = <int, CirPlotSample>{};
        for (final sample in samples) {
          latestByAnchor.putIfAbsent(sample.anchorId, () => sample);
        }
        return Card(
            ...
            child: Column(
              ...
              children: [
                Row(
                  children: [
                    ...
                    StatusPill(
                      ...
                      tone: samples.isEmpty ? PillTone.neutral : PillTone.good,
                    ),
                  ],
                ),
                const SizedBox(height: 12),
                Container(
                  ...
                  child: latestByAnchor.isEmpty
                      ? const Center(
                          child: Text(
                            'No full CIR frames yet',
                            style: TextStyle(color: mutedText),
                          ),
                        )
                      : CustomPaint(
                          painter: CirWaveformPainter(latestByAnchor),
                        ),
                ),
              ],
            ),
...
class CirWaveformPainter extends CustomPainter {
  CirWaveformPainter(this.latestByAnchor);

  final Map<int, CirPlotSample> latestByAnchor;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final gridPaint = Paint()
      ..color = tableLine.withValues(alpha: 0.18)
      ..strokeWidth = 0.8;
    final baselinePaint = Paint()
      ..color = controlGreen.withValues(alpha: 0.18)
      ..strokeWidth = 1.0;
    final textPainter = TextPainter(textDirection: TextDirection.ltr);
    final rowHeight = size.height / 8.0;
    const leftPad = 86.0;
    const rightPad = 18.0;
    final plotWidth = (size.width - leftPad - rightPad).clamp(1.0, size.width);
    const colors = [
      controlGreen,
      biospurGlow,
      Color(0xFF7DD3FC),
    ...
    final bgPaint = Paint()..color = const Color(0xAA020602);
    ...
    for (var i = 0; i <= 12; i++) {
      final x = leftPad + plotWidth * i / 12.0;
      canvas.drawLine(Offset(x, 0), Offset(x, size.height), gridPaint);
    }

    for (var anchor = 0; anchor < 8; anchor++) {
      final sample = latestByAnchor[anchor];
      final top = anchor * rowHeight;
      final baseY = top + rowHeight * 0.82;
      final traceTop = top + rowHeight * 0.12;
      canvas.drawLine(
        Offset(leftPad, baseY),
        Offset(size.width - rightPad, baseY),
        baselinePaint,
      );
      textPainter.text = TextSpan(
        text: '${anchorLabelForId(anchor)}\n${sample?.rawDistanceMm ?? '-'} mm',
        style: const TextStyle(
          color: controlGreen,
          fontSize: 12,
          fontWeight: FontWeight.w700,
      ...
      textPainter.layout(maxWidth: leftPad - 14);
      textPainter.paint(canvas, Offset(10, top + 8));
      ...
      final fillPath = Path();
      final points = <Offset>[];
      final vals = sample.values;
      for (var i = 0; i < vals.length; i++) {
        final x = vals.length == 1
            ? leftPad
            : leftPad + i * plotWidth / (vals.length - 1);
        final norm = math.sqrt(vals[i].clamp(0, 1000) / 1000.0);
        final y = baseY - norm * (baseY - traceTop);
      ...
      fillPath.lineTo(leftPad + plotWidth, baseY);
      fillPath.close();
      glowPaint.color = colors[anchor].withValues(alpha: 0.22);
      tracePaint.color = colors[anchor].withValues(alpha: 0.98);
      pointPaint.color = colors[anchor].withValues(alpha: 0.88);
      barPaint.color = colors[anchor].withValues(alpha: 0.28);
      fillPaint.color = colors[anchor].withValues(alpha: 0.035);
      canvas.drawPath(fillPath, fillPaint);
      for (final point in points) {
        canvas.drawLine(Offset(point.dx, baseY), point, barPaint);
      }
      canvas.drawPath(path, glowPaint);
      canvas.drawPath(path, tracePaint);
      canvas.drawPoints(ui.PointMode.polygon, points, tracePaint);
      canvas.drawPoints(ui.PointMode.points, points, pointPaint);

      final peakIndex = vals.indexOf(vals.reduce((a, b) => a > b ? a : b));
      final peakX = vals.length == 1
          ? leftPad
          : leftPad + peakIndex * plotWidth / (vals.length - 1);
      final peakNorm = math.sqrt(vals[peakIndex].clamp(0, 1000) / 1000.0);
      final peakY = baseY - peakNorm * (baseY - traceTop);
      final peakPaint = Paint()
        ..color = colors[anchor].withValues(alpha: 0.95)
        ..style = PaintingStyle.fill;
      canvas.drawCircle(Offset(peakX, peakY), 4.8, peakPaint);
    }
  }

  @override
  bool shouldRepaint(covariant CirWaveformPainter oldDelegate) {
    return oldDelegate.latestByAnchor != latestByAnchor;
  }
}

class CirAnchorTile extends StatelessWidget {
  const CirAnchorTile({super.key, required this.sample, required this.index});

  final CirCrxSample? sample;
  final int index;

  @override
  Widget build(BuildContext context) {
    final s = sample;
    final label = anchorLabelForId(index);
    final tone = s == null
        ? PillTone.neutral
        : s.qualityScore >= 0.72
        ? PillTone.good
        ...
        children: [
          Row(
            children: [
              ...
              const Spacer(),
              StatusPill(label: '', value: s?.qualityLabel ?? '-', tone: tone),
            ],
          ),
          const SizedBox(height: 10),
          LinearProgressIndicator(
            value: s?.qualityScore ?? 0,
            minHeight: 8,
            backgroundColor: const Color(0x55384515),
            valueColor: AlwaysStoppedAnimation<Color>(toneColor(tone)),
          ),
          const SizedBox(height: 10),
          Text('raw ${s?.rawDistanceMm ?? '-'} mm'),
          Text('fp ${s?.fpAmp ?? '-'}  peak ${s?.maxGrowthCir ?? '-'}'),
          Text('noise ${s?.stdNoise ?? '-'} / ${s?.maxNoise ?? '-'}'),
        ],
      ),
    );
  }
...
class CirCrxSample {
  const CirCrxSample({
    ...
    required this.rxTimestamp,
    required this.carrierIntegrator,
    required this.firstPath,
    required this.firstPathAmp1,
    required this.firstPathAmp2,
    required this.firstPathAmp3,
    required this.maxGrowthCir,
    required this.rxPreamCount,
  ...
  final int rxTimestamp;
  final int carrierIntegrator;
  final int firstPath;
  final int firstPathAmp1;
  final int firstPathAmp2;
  final int firstPathAmp3;
  final int maxGrowthCir;
  final int rxPreamCount;
  final int stdNoise;
  final int maxNoise;

  int get fpAmp => firstPathAmp1 + firstPathAmp2 + firstPathAmp3;

  double get qualityScore {
    if (rawDistanceMm <= 0) return 0;
    final ampScore = (fpAmp / 2400.0).clamp(0.0, 1.0);
    final noisePenalty = (stdNoise / 140.0).clamp(0.0, 1.0);
    final peakScore = (maxGrowthCir / 4500.0).clamp(0.0, 1.0);
    return (0.62 * ampScore + 0.28 * peakScore + 0.10 * (1.0 - noisePenalty))
        .clamp(0.0, 1.0);
  }

  String get qualityLabel {
    if (rawDistanceMm <= 0) return 'bad';
  ...
  static List<CirCrxSample> fromLines(List<String> lines) {
    final out = <CirCrxSample>[];
    for (final line in lines.reversed) {
      final sample = parse(line);
      if (sample != null) out.add(sample);
    }
    return out;
  }

  static CirCrxSample? parse(String line) {
    final cirpIdx = line.indexOf('CIRP;');
    if (cirpIdx >= 0) {
      final parts = line.substring(cirpIdx).split(';');
      if (parts.length < 10 || parts[0] != 'CIRP' || parts[1] != '1') {
        return null;
      }
      int? at(int i) => int.tryParse(parts[i].trim());
      final sweep = at(2);
      final anchor = at(3);
      final raw = at(4);
      final fp = at(5);
    ...
    final acirmIdx = line.indexOf('ACIRM;');
    if (acirmIdx >= 0) {
      final parts = line.substring(acirmIdx).split(';');
      if (parts.length < 15 || parts[0] != 'ACIRM' || parts[1] != '1') {
        return null;
      }
      int? at(int i) => int.tryParse(parts[i].trim());
      final sweep = at(2);
    ...
    final acrxIdx = line.indexOf('ACRX;');
    if (acrxIdx >= 0) {
      final parts = line.substring(acrxIdx).split(';');
      if (parts.length < 18 || parts[0] != 'ACRX' || parts[1] != '1') {
        return null;
      }
      int? at(int i) => int.tryParse(parts[i].trim());
      final sweep = at(2);
      ...
      final peak = at(14);
      final pream = at(15);
      final stdNoise = at(16);
      final maxNoise = at(17);
      if ([
        sweep,
        anchor,
        raw,
    ...
    final cirmIdx = line.indexOf('CIRM;');
    if (cirmIdx >= 0) {
      final parts = line.substring(cirmIdx).split(';');
      if (parts.length < 12 || parts[0] != 'CIRM' || parts[1] != '1') {
        return null;
      }
      int? at(int i) => int.tryParse(parts[i].trim());
      final sweep = at(2);
    ...
    final idx = line.indexOf('CRX;');
    if (idx < 0) return null;
    final parts = line.substring(idx).split(';');
    if (parts.length < 15 || parts[0] != 'CRX' || parts[1] != '1') {
      return null;
    }
    int? at(int i) => int.tryParse(parts[i].trim());
    final sweep = at(2);
...
class CirPlotSample {
  ...
  static List<CirPlotSample> fromLines(List<String> lines) {
    final out = <CirPlotSample>[];
    for (final line in lines.reversed) {
      final sample = parse(line);
      if (sample != null) out.add(sample);
    }
  ...
  static CirPlotSample? parse(String line) {
    final idx = line.indexOf('CIRP;');
    if (idx < 0) return null;
    final parts = line.substring(idx).split(';');
    if (parts.length < 11 || parts[0] != 'CIRP' || parts[1] != '1') {
      return null;
    }
    int? at(int i) => int.tryParse(parts[i].trim());
    final sweep = at(2);
    ...
    final peak = at(7);
    final noise = at(8);
    final values = parts[10]
        .split(',')
        .map((v) => int.tryParse(v.trim()))
        .whereType<int>()
        .toList();
    if ([sweep, anchor, raw, fp, fpAmp, peak, noise].any((v) => v == null) ||
        values.isEmpty) {
      return null;
    }
    return CirPlotSample(
      sweep: sweep!,
      anchorId: anchor!,
      rawDistanceMm: raw!,
      firstPath: fp!,
...
class PortReader {
  static Future<PortSnapshot> read() async {
    ...
    final cirUsbCount = cirUsbPorts.values
        .where((path) => names.contains(path.split('/').last.replaceFirst('usb-', '')))
        .length;
    return PortSnapshot(
      masterAnchor:
      ...
      cirTagUsb: names.contains('SEGGER_J-Link_000760186115'),
      cirUsbCount: cirUsbCount,
    );
  }
}
...
class PortSnapshot {
  const PortSnapshot({
    ...
    required this.cirTagUsb,
    required this.cirUsbCount,
  });

  final bool masterAnchor;
  final bool masterTag;
  final bool cirTagUsb;
  final int cirUsbCount;

  bool get anyMaster => masterAnchor || masterTag || cirUsbCount > 0;
