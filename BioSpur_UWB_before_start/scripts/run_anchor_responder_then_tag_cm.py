...
def run_tag_cm_100(
    ...
    if not ok:
        ...
        return ser, False
    ser, oneshot_txt = send_cmd_collect_text(ser, logf, port, "oneshot MCAL", 0.9, True, 1)
    if "oneshot rc=-128" in oneshot_txt or "target mismatch" in oneshot_txt:
        logf.write("PRECHECK FAIL: oneshot MCAL rejected (target mismatch / rc=-128)\n")
        return ser, False
    ...
    did_recover = False
    no_cm_grace_s = 20.0
    no_cm_deadline = time.time() + no_cm_grace_s
    while time.time() < deadline:
        try:
            data = ser.read(4096)
        except (SerialException, OSError):
        ...
        # If we see no CM at all after a grace window, try one recover step.
        if not did_recover and count == 0 and time.time() > no_cm_deadline:
            did_recover = True
            logf.write(f"\nCM_CAPTURE_NO_DATA after {no_cm_grace_s:.0f}s; trying conn+oneshot recover\n")
            ser = send_cmd_collect(ser, logf, port, "conn", 1.0, True, 1)
            ser, ok = ensure_ota_target_name(ser, logf, port, target_name, True, 1, retries=3)
            if not ok:
                logf.write("RECOVER FAIL: ota_target not stable after conn\n")
                return ser, False
            ser, oneshot_txt = send_cmd_collect_text(ser, logf, port, "oneshot MCAL", 0.9, True, 1)
            if "oneshot rc=-128" in oneshot_txt or "target mismatch" in oneshot_txt:
                logf.write("RECOVER FAIL: oneshot MCAL rejected\n")
                return ser, False
