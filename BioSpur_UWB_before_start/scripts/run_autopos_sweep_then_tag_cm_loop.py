    ...
    try:
        ...
        with open(log_path, "w", buffering=1) as logf:
            ...
            while time.time() < mode_deadline:
                ...
                text = data.decode("utf-8", "ignore")
                logf.write(text)
                if f"{target_name} notify: MODE_OK MODE=CAL LIVE=1" in text:
                    result["mode_ok_seen"] = True
                    break

            if not result["mode_ok_seen"]:
                result["error"] = "mode_ok_not_seen"
                return result
...
def main() -> int:
    ...
    while args.loops == 0 or iteration < args.loops:
        ...
        for master in args.order:
            ...
            result = round_capture(args.port, master, round_dir, args.timeout_s)
