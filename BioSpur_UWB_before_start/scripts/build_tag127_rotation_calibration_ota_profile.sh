...
# Build OTA-deliverable Tag127 rotation-calibration profile + matching master_ota image.
# This is used after Ref115 static calibration is deactivated from UWB.
#
# Usage:
#   ./scripts/build_tag127_rotation_calibration_ota_profile.sh [tag_build_dir] [master_ota_build_dir]
#
# Environment overrides:
#   TAG_TOKEN_ID (default: 206)  # current token for device 760186127
#   TAG_LOGICAL_ID (default: 2)
#   TAG_DEVICE_NAME (default: BS_AUTO)
#   TAG_SIGN_VERSION (default: 0.0.1+127)

TAG_BUILD_DIR="${1:-build-tag-ota-tag127-rotation-calibration}"
MASTER_BUILD_DIR="${2:-build-master-ota-tag127-rotation-calibration}"

TAG_TOKEN_ID="${TAG_TOKEN_ID:-206}"
TAG_LOGICAL_ID="${TAG_LOGICAL_ID:-2}"
TAG_DEVICE_NAME="${TAG_DEVICE_NAME:-BS_AUTO}"
TAG_SIGN_VERSION="${TAG_SIGN_VERSION:-0.0.1+127}"
...
-DAPP_TAG_FW_MARKER=\"tag127-rotation-calibration-ota\" \
-DAPP_TAG_CALIBRATION_MODE=1 \
-DAPP_TAG_UWB_ENABLE=1 \
...
echo "[tag127 rotation calibration ota] token=${TAG_TOKEN_ID} logical_id=${TAG_LOGICAL_ID}"
echo "[tag127 rotation calibration ota] tag_build_dir=${TAG_BUILD_DIR}"
echo "[tag127 rotation calibration ota] master_ota_build_dir=${MASTER_BUILD_DIR}"
...
python3 scripts/write_build_source.py \
  ...
  --source "scripts/build_tag127_rotation_calibration_ota_profile.sh" \
  --command "$0 $* (tag127 rotation calibration ota profile)"

python3 scripts/write_build_source.py \
  --build-dir "$MASTER_BUILD_DIR" \
  --source "scripts/build_tag127_rotation_calibration_ota_profile.sh" \
  --command "$0 $* (master ota for tag127 rotation calibration profile)"

echo
echo "Built Tag127 OTA rotation-calibration profile."
