...
\textit{This is the mechanism-paper novelty assessment (Paper~B, an identifiability/error-analysis
paper for a metrology venue); its companion is the AutoPos system-paper assessment (Paper~A). The
split is deliberate and non-overlapping: Paper~A reports the working, ground-truth-validated system;
Paper~B explains \emph{why} its residual vertical error exists and why added geometry cannot
remove it.}

\section{One-Sentence Thesis}

UWB anchor self-calibration couples antenna-delay/range-bias parameters with layout scale, which makes same-environment tag accuracy unreliable as a calibration-quality metric. In the Erlangen campaign the \texttt{v4-io} layout is scale-biased (metric-incorrect): an under-absorbed common-mode range bias expands it to an AutoPos-to-Vicon Sim(3) scale of 0.958---this is a range-bias signature, not a true geometric distortion. This scale-biased layout gives a better static median (72.7~mm) than the metric-correct common-mode layout (109.5~mm) \emph{only when the tag delay is uncorrected}, because the layout's scale bias compensates an uncorrected positive tag-device delay. The effect is conditional: once the tag delay is corrected with a single global constant the metric-correct layout \emph{wins} (68.5~mm held-out median, P95 156~mm, versus the scale-biased layout's 72.7~mm), so the claim is not that a metric-incorrect layout is inherently a better positioner, but that same-environment accuracy can rank a scale-biased layout above an independently validated metric-correct one when a correctable tag/range bias is left uncorrected.
...
recovered Sim(3) scale $0.958$ (a positive common bias shrinks the layout, matching the
negative alias sign). One honest caveat: the globally-softest Fisher modes of this thin
...
compensates an uncorrected positive tag-device delay. The decisive fair-comparison control settles it: the physically honest correction is a \emph{single global}
...
positions) the metric-correct layout \emph{beats} the scale-biased one: 68.5~mm held-out
median (P95 156~mm) versus 72.7~mm, and with a far more stable tail than per-frame joint
estimation, which only ties the median (72.9~mm) by overfitting and blows the tail to
P95 317.5~mm. This closes the causal chain: the scale-biased layout's apparent advantage
...
differences that drive the coupling. The per-device delays are thus unavailable from factory data and
recoverable only by per-unit bench calibration (not done in deployment) or self-calibration, which
entangles them with geometry --- the effect studied here. The coupling is intrinsic to commodity
single-antenna hardware, not an artifact of a skipped calibration step.
...
flips the in-environment ranking so the metric-correct layout wins ($68.5$ vs $72.7$~mm), establishing
...
accuracy. For defensive-publication value the manuscript states the enabling combination once ---
...
global delay constant is applied --- after which the metric-correct layout wins.''}
