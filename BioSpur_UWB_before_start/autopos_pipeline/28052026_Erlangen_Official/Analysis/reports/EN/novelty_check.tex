...
UWB anchor self-calibration couples antenna-delay/range-bias parameters with layout scale, which makes same-environment tag accuracy unreliable as a calibration-quality metric. In the Erlangen campaign the \texttt{v4-io} layout is scale-biased (metric-incorrect): an under-absorbed common-mode range bias expands it to an AutoPos-to-Vicon Sim(3) scale of 0.958---this is a range-bias signature, not a true geometric distortion. This scale-biased layout gives a better static median (72.7~mm) than the metric-correct common-mode layout (109.5~mm) \emph{only when the tag delay is uncorrected}, because the layout's scale bias compensates an uncorrected positive tag-device delay. The effect is conditional: once the tag delay is corrected the metric-correct layout matches it (72.9~mm median), so the claim is not that a metric-incorrect layout is inherently a better positioner, but that same-environment accuracy can rank a scale-biased layout above an independently validated metric-correct one when a correctable tag/range bias is left uncorrected.

\section{Scope of the Literature Check}

The first version of this note relied mainly on abstract-level metadata. This revision rechecked every locally available PDF and then expanded the corpus with additional open-access self-calibration and delay-calibration papers. Zotero contained 156 items but no PDF attachments, so the full-text corpus came from the downloaded literature folder. Forty-one registry entries could be converted to text and were searched page by page for delay--geometry coupling, identifiability, Fisher/CRB language, error cancellation, common-mode effects, metric correctness, similarity alignment, post-selection, and ground-truth evaluation. One Patwari PDF was available only as scanned page images. The De Preter 2019 proceedings full text is available (LIRIAS) and is one of the closest prior-art records: by its formulation it jointly estimates anchor coordinates, tag coordinates, and a constant range-bias model in the same autocalibration problem, so it must be compared line-by-line in the manuscript (see the Antenna Delay subsection below). Hesch 2014 remains abstract-only after a targeted access search. The expanded pass added AniTrack as a consistent independent observation: it reported a self-localized-beats-surveyed pattern but did not explain the mechanism, and its sample is small (seven tag positions) \citep{luder_2025_anitrack}.
...
and scale. We are explicit that this evidence shows the delay--scale direction is
\emph{weakly observable yet still identifiable} in this non-coplanar deployment
(the common mode is recoverable, the minimum is unique), \emph{not} strictly
degenerate. A formal Fisher-information / profile-likelihood analysis of this
direction is the planned next step and is required before any near-degeneracy is
claimed; the cost-clamp ratio is reported here only as a practical indicator, not as
an identifiability proof. The two analyses operate on different parameter spaces and
answer different questions. As an independent empirical check that this
vertical/scale weakness is a delay effect rather than something a richer
calibration dataset could remove, we ran an active vertical-information injection:
a tilted two-tag rotating arm swept the rotation plane from horizontal to vertical
during self-calibration. The eight-anchor two-layer array already has
well-conditioned vertical geometry, so no observability deficit is expected;
consistent with this, injecting strong vertical excitation did not reduce the
recovered anchor vertical error and showed no dose--response with rotation tilt
under either a free-point or a rigid-body arm model, indicating the weak direction
is dominated by per-device delay coupling rather than by anything additional
vertical calibration data could supply (see the supplementary experimental
report).
...
Several prior systems use ground truth, motion capture, or external trajectory sources for evaluation \citep{lutz_2019_visual_inertial_slam_uwb_error_model,ledergerber_2018_calibrating_away_inaccuracies_uwb,queralta_2020_uwb_based_system_for_uav}. What is less common is an explicit separation between anchor-side metric correctness and tag-side same-environment positioning accuracy. The Erlangen analysis uses Sim(3) scale, transfer matrices, winner's-curse correction, nested spatial validation, phase-center sensitivity, and raw-frame ranking reversal as separate checks. This is a measurement-validity protocol rather than a new real-time localization algorithm.
...
scale recovery and a cost-clamp test (with a formal observability analysis planned);
...
This paper makes two contributions; the methodological falsification protocol (the
second contribution, below) is the headline claim, and the empirical coupling result
is framed as characterization rather than first discovery.
...
noted in UWB \citep{hamer_2018_self_calibrating_ultra_wideband_network} and in
acoustic array calibration \citep{burgess_2015_toa_self_calibration}. We provide
...
direction (a formal Fisher/observability analysis is planned). We then show that the
metric-correct common-mode layout is the worse same-environment positioner (109.5~mm
median) than the scale-biased \texttt{v4-io} layout (72.7~mm) \emph{only when the tag
delay is left uncorrected}: the scale-biased layout's scale error compensates an
uncorrected positive tag-device delay. Per-frame joint tag-delay estimation restores
the common-mode layout to 72.9~mm (a tie on median) but destabilizes the tail
(P95 317.5~mm). We flag the fair-comparison control still required---the metric-correct
layout with a single global tag-delay constant---so that the win is not attributed to
geometry when it may reduce to an uncorrected constant offset.
...
physical calibration; this intervention belongs in the manuscript main evidence chain,
not in an appendix or on request.
...
coupling is not the claimed novelty. The novelty is the explicit Vicon-grounded
characterization in UWB self-calibration, the demonstration that the coupling can
make the scale-biased layout win in-environment under an uncorrected tag
bias, and the diagnostic intervention that confirms cancellation. We did not find
these elements combined in the acoustic or GNSS prior art, though we cite that work
as closely related. The paper should
