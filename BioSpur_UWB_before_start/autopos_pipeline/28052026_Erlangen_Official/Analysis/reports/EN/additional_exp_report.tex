...
This document supplements the original English Vicon validation report and records the experiments that were completed after that report was sent for review. The original campaign left a central paradox: the V5 common-mode anchor layout corrected the anchor-side metric scale, with Sim3 scale moving from 0.958 for V4 to 1.010 for V5, yet the older V4 layout still gave the lower p50 static median on the same 24-position Erlangen campaign. Under the unified scalar convention, V4+C\_V4+D\_LOO gives 61.5 mm median 3D error, while V5+C\_V5+D\_LOO gives 63.5 mm. The gap is only 2.0 mm, so the baseline paradox is narrower than the earlier mixed-convention table implied. These values are reproduced from \texttt{FULL\_V5\_convention\_unification/tables/unified\_headline\_table.csv}.
...
The caveat is the tail. Under the unified scalar convention, the lower-quantile V5 row improves the median to 44.5 mm and improves P95 from 175.1 mm for V5 scalar p50 to 164.1 mm, with RMSE 81.5 mm. This removes the old apparent V4 P95 advantage, which came from mixing evaluation conventions. The tail is still high in absolute terms, so the lower-tail result solves typical static error better than it solves worst-case static positioning. The 5000-iteration bootstrap interval, 33.4--82.8 mm, also remains wide because the campaign has only 24 static positions.
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{literature_search/rotarm_zinjection_rigid_dose_response.png}
\caption{Rigid-arm vertical-injection dose--response against motion-capture rotation tilt. The mechanism predicts a downward slope toward $90^\circ$; the observed dependence is flat and slightly positive, every tilt bin is worse than the no-injection baseline, and the near-vertical bin is the worst. Source: \texttt{literature\_search/rotarm\_zinjection\_rigid\_dose\_response.png}.}
\label{fig:rotarm}
\end{figure}

The result confirms that the systematic vertical error is not reducible by richer vertical calibration data. Actively injecting strong vertical excitation does not reduce the vertical error and, if anything, slightly increases it, whereas tag-delay correction reduces the vertical static error substantially (quantified below). This is exactly what is expected when the layout's vertical observability is already good: there is no information deficit for the injection to fill, so the residual vertical bias is the per-tag projection of a common-mode range bias rather than a missing-geometry effect. A geometric dilution-of-precision analysis of the actual layout makes this concrete: over the static tag positions a common-mode range bias aliases into the vertical coordinate about $6.9\times$ more strongly than into the horizontal plane (median vertical-to-horizontal alias gain $1.58$ versus $0.23$\,mm per mm of delay), with $\mathrm{VDOP}/\mathrm{HDOP}\approx1.6$ -- squarely in the GPS receiver-clock/altitude regime. A representative tag antenna delay of order $35$\,mm therefore predicts a vertical bias of order $55$\,mm against only about $8$\,mm horizontal, consistent in sign, direction, and order of magnitude with the measured drop of the vertical static error from $61.9$ to $39.3$\,mm under tag-delay correction while the horizontal error is essentially unchanged (geometry analysis in \texttt{literature\_search/delay\_vertical\_dop\_results.txt}). This alias is confirmed empirically (Figure~\ref{fig:delayinject}): injecting a known common-mode offset into the static ranges and re-localizing without modelling it shifts the recovered vertical coordinate at $1.61$\,mm per mm of injected delay, matching the closed-form prediction of $1.58$, against only $0.23$\,mm horizontally; and in a complementary test against motion-capture truth, correcting a single common delay reduces the vertical localization error by roughly an order of magnitude more than the horizontal, confirming the vertical-specific action of the correction. The value of this result is as a confirmation that closes a data-coverage alternative and identifies the mechanism, not as the discovery of a new effect. The absolute error levels here come from a re-analysis pipeline whose baseline, 68.8 mm, is slightly worse than the production layout at 59.8 mm, so only the internal with-and-without and tilt-dose comparisons are used as evidence.

\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{literature_search/delay_injection_sweep.png}
\caption{Empirical confirmation of the common-delay-to-vertical alias. Injecting a known common-mode range offset $\Delta b$ into the static ranges and re-localizing without modelling it shifts the recovered position: the vertical shift tracks $\Delta b$ at slope $1.61$ (closed-form geometric prediction $1.58$), while the horizontal shift grows at only $0.23$ per unit $|\Delta b|$, the UWB analogue of the GPS receiver-clock/altitude coupling. Source: \texttt{literature\_search/delay\_injection\_sweep.png}.}
\label{fig:delayinject}
\end{figure}
...
The headline table below is reproduced from \texttt{FULL\_V5\_convention\_unification/tables/unified\_headline\_table.csv}. It should be read as a campaign registry rather than a single leaderboard, because rows differ in evaluation protocol. Rows labelled in-sample, diagnostic, Sim3, or oracle should not be used as deployment claims. Row O is the new raw-frame static batch result; its median is strong, and under the scalar convention its P95 is slightly better than the V5 p50 baseline, although still high in absolute terms.
...
B & V4 + D\_LOO & 61.5 & 157.0 & 87.5 & LOO-CV &  \\
...
The most important comparison is not Row O against every other row, but the ranking reversal under a controlled change in tag-anchor range bias. Under scalar p50, V4 wins only narrowly at 61.5 mm versus V5 at 63.5 mm. Under lower\_trim\_20 with Huber30, V5 reaches 44.5 mm and the best V4 raw-frame geometry row is 53.9 mm. That reversal is the evidence for delay--layout--NLOS coupling.
...
30 & A & Active vertical-geometry injection by tilted arm rotation does not reduce the vertical anchor error (dose--response slope $+0.03$ mm/deg, both solver models); since the cuboid layout's vertical observability is already good (a common-mode tag delay aliases about $7\times$ more into vertical than horizontal, $\mathrm{VDOP}/\mathrm{HDOP}\approx1.6$), this confirms the residual vertical bias is a per-tag delay-to-altitude alias -- the UWB analogue of GPS clock--altitude coupling, distinct from the anchor-side delay--scale coupling -- not a quantity reducible by richer vertical calibration data. \\
\bottomrule
\end{tabular}
\end{table}
...
The first open question is tail reduction. The lower-quantile row improves the static median and improves P95 relative to scalar V5 p50, but P95 remains around 164 mm. The next analysis should identify which positions and anchors cause this tail and whether per-anchor stability rules can reduce it without giving back the median gain.
