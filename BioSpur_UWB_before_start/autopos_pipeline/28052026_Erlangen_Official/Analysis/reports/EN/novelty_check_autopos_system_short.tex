...
geometric: with a single coplanar layer the vertical is badly ill-conditioned --- the height still
falls out, just far too inaccurate to use. The minimal remedy is the intuitive one --- four anchors is
...
array that, \emph{within reference-free self-calibration}, restores the well-conditioned vertical geometry that
a single coplanar layer lacks --- deployed few-anchor raises avoid that wall only
by surveying the raised positions, not self-calibrating them (a self-calibration Fisher analysis gives
a vertical CRLB exactly singular for a coplanar layer and $\sim$63~mm for $4{+}4$, at the $\sim$60~mm
production floor); and (iii) two
ground-truth-free deployment diagnostics --- that ranging residual is a misleading quality signal
under minimal anchor counts, and that dispersion across dynamic anchor selection exposes calibration
bias that a fixed anchor set would hide.
...
The \emph{vertical} is the point: $\sim$4--6~cm from pure UWB, on an axis 32 of 37 reviewed systems
...
\textbf{Honest limits.} The preliminary $\sim$5~cm figures are \emph{repeatability}, not accuracy ---
...
its delay--layout coupling result. These figures also presuppose the
eight-anchor, two-layer \textbf{broadcast SS-TWR} acquisition the array is built for: a naive
minimal-anchor SS-TWR shortcut, or a single coplanar layer, returns the vertical to its singular
regime by construction and will not reproduce them.

\textbf{Priority and disclosure.} To our knowledge this is the first demonstration that the
\emph{vertical} axis of a UWB anchor array can be self-calibrated to the centimetre level
($\sim$59~mm absolute vs Vicon, near a $\sim$63~mm inter-anchor Cram\'{e}r--Rao floor) from
inter-anchor ranging alone on commodity single-antenna modules --- the number later reference-free
pure-UWB vertical self-calibration must beat. For defensive-publication value the manuscript discloses
the full enabling combination once (3D self-calibration; inter-anchor ranging only; joint per-device
antenna-delay estimation; commodity DWM1001C modules; balanced $4{+}4$ array; optical-ground-truth
validation --- no survey, no external $Z$ sensor, no mobile platform), and ships the CRLB script and
ranging data so the floor is independently reproducible. Publish this system paper first and early
(dated, indexed) so it becomes the citable prior work the companion paper builds on.

\textbf{Venue.} IEEE Sensors / IEEE Access / IPIN (full paper); the higher-tier Measurement / IEEE
TIM target is reserved for the companion mechanism paper. Paper~A first, to establish the validated
