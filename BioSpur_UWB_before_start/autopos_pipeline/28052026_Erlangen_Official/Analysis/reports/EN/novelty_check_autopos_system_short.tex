...
array that, \emph{within reference-free self-calibration, and only under the eight-anchor broadcast SS-TWR acquisition it is dimensioned for}, restores the well-conditioned vertical geometry that
...
a vertical CRLB exactly singular for a coplanar layer and $\sim$63~mm for $4{+}4$, at the ${\approx}62$~mm
deployed median vertical error); and (iii) two
...
regime by construction and will not reproduce them. All figures are from a single room with 24 static
tag positions; multi-room transfer is unproven. The headline median also benefits, in part, from the
production layout's expanded scale compensating an uncorrected tag-side delay --- the companion paper's
mechanism; with that delay left at zero the metric-correct layout gives a larger median.
...
(${\approx}62$~mm median absolute vs Vicon, near a $\sim$63~mm inter-anchor Cram\'{e}r--Rao floor) from
inter-anchor ranging alone on commodity single-antenna modules --- a reference point that later reference-free
pure-UWB vertical self-calibration in this regime can be measured against. For defensive-publication value the manuscript discloses
