...
Generated 2026-06-04T14:45:59.678142+00:00.
...
| Numerical precision: raw-pair median bootstrap original_selfcal | 1.02 | 0.76 | 0.56 | 1.13 | numerical_precision |
| Numerical precision: raw-pair median bootstrap vicon_truth_delaycal | 0.00 | 0.00 | 0.45 | 0.00 | numerical_precision |
| Numerical precision: raw-pair median bootstrap scale_to_vicon_delaycal | 0.97 | 0.70 | 0.53 | 1.08 | numerical_precision |
| Numerical precision: raw-pair median bootstrap one_baseline_EH_delaycal | 1.20 | 1.24 | 0.54 | 1.29 | numerical_precision |

### Tag Static
| layout_delay_config | repeatability_3d_sd_median_mm | absolute_3d_rmse_mm | x_rmse_mm | y_rmse_mm | z_rmse_mm | absolute_3d_p95_mm |
| --- | --- | --- | --- | --- | --- | --- |
| Legacy production mean-aggregated static point v4-io/T1 |  | 139.55 | 38.48 | 130.54 | 30.88 | 282.13 |
| Original FULL median-estimator ablation v4-io/T4 | 67.44 | 108.92 | 37.57 | 97.95 | 29.29 | 173.81 |
| Original FULL filtered static v4-io/T4+F5 | 18.68 | 109.51 | 37.00 | 98.90 | 29.00 | 175.69 |
| Original FULL filtered static v4-io/T3+F5 | 20.97 | 105.38 | 43.59 | 89.78 | 33.85 | 170.10 |
...
| --- | --- | --- | --- | --- | --- |
| AutoPos v4-io rigid | solver residual corrections | 108.93 | 174.07 | 48.17 | current self-cal co-fit output; geometry and layout-level residual delay corrections are coupled |
...
| Vicon/OptiTrack truth anchors | re-estimated inter-anchor residual corrections | 77.67 | 128.38 | 59.94 | known-anchor + re-estimated-delay control; best static control and better than self-cal, not a lower bound on achievable error |
...
| case | layout_frame | delay_treatment | tag_rmse_mm | tag_p95_mm | claim_role |
| --- | --- | --- | --- | --- | --- |
| truth_coords_no_delay | Vicon/OptiTrack truth anchors | none | 311.32 | 453.45 | optical geometry alone is not sufficient |
| truth_coords_transplanted_selfcal_delay | Vicon/OptiTrack truth anchors | AutoPos self-cal layout-level residual corrections transplanted | 252.17 | 394.57 | residual corrections are layout-frame conditioned, not transferable |
| truth_coords_reestimated_delay | Vicon/OptiTrack truth anchors | re-estimated inter-anchor residual corrections | 77.67 | 128.38 | known-anchor + re-estimated-delay control |
| autopos_selfcal_rigid_solver_delay | AutoPos v4-io rigid no-scale | self-cal layout-level residual corrections | 108.92 | 173.81 | self-cal is competitive because geometry and residual corrections are co-fitted |

The `truth_coords_no_delay`, `truth_coords_transplanted_selfcal_delay`, and `truth_coords_reestimated_delay` rows isolate the same tag data under the imposed optical coordinate frame. The result is the 311/252/77 mm RMSE triangle: ground-truth coordinates alone are not enough, and layout-level residual delay corrections must be re-estimated on the coordinate frame being solved. This is the narrow contribution claim; it is not a novelty claim about generic joint geometry/delay fitting.

Mechanism view: WHY #9 regresses self-minus-Vicon `anchor_main_rel_A` on v4-io radial layout error and obtains R2= with slope  mm/mm (``). Outcome view: the same coupling appears as the 311/252/77 tag-RMSE triangle above. Together they explain why the self-cal layout can be geometrically off by about 105 mm rigid SE(3) RMSE yet remain ranging-competitive: the self-cal residual-delay term absorbs radial coordinate/scale error, so it is not a physical per-anchor delay and not a solver bug.
...
- `../tables/delay_layout_coupling.csv`
- `../tables/checklist_coverage.csv`
- `../figs/delay_layout_coupling.png`
