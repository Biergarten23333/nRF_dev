...
def plot_group_metric(path: Path, rows: list[dict], group_col: str, title: str) -> None:
    df = pd.DataFrame(rows)
    fig, axs = plt.subplots(1, 2, figsize=(11, 4.5), sharey=True, constrained_layout=True)
...
def build_summary_md(
    *,
    ...
    lines.append("Height grouping uses bootstrap CIs because each group is small. The output table reports 3D median/p95 and OptiTrack X/Y/Z split for all8/noG.\n\n")
...
def main() -> int:
    ...
    dop = dop[(dop["version"] == "v4-io") & (dop["mask"].isin(["all8", "noG"]))]
