...
def main() -> int:
    ...
    opti_dir = Path(args.opti_dir).resolve() if args.opti_dir else official_root / "opti_captures/static"
    ...
    write_csv(tables_dir / "layout_abs_errors_noG.csv", all_error_rows["noG"])
    ...
    md.append(
        "G marker warning: Gshort/Glong/marker-fingerprint is treated as suspect. "
        "This script writes `opti_anchor_marker_fingerprint.csv`, does not reconstruct "
        "G from the jig, and reports all headline layout numbers both all8 and noG.\n\n"
    )
    v4_all8 = [r for r in all_summary if r["version"] == "v4-io" and r["eval_set"] == "all8"]
    v4_nog = [r for r in all_summary if r["version"] == "v4-io" and r["eval_set"] == "noG"]
    if v4_all8:
        r = v4_all8[0]
        md.append(
    ...
    if v4_nog:
        r = v4_nog[0]
        md.append(
            f"No-G headline: v4-io noG rigid RMS {r['reflection_allowed_rms_3d_mm']:.1f} mm "
            f"(horizontal {r['reflection_allowed_horizontal_rms_mm']:.1f} mm, "
            f"vertical {r['reflection_allowed_vertical_rms_mm']:.1f} mm).\n\n"
        )
    md.append("## Summary\n\n")
