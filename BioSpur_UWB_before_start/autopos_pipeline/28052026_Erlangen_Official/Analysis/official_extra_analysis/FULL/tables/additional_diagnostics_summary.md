...
The all8 3D-error slope is 166.8 mm/m (R^2=0.27, p=0.009), which supports a positive scale-propagation component. The signed radial slope is 254.5 mm/m (p=0.000). The moderate R^2 means scale propagation is real but not the whole tail explanation.
...
The all8 mean error vector is (22.6, -19.7, -6.3) mm in OptiTrack XYZ, with |mean|/RMS-scatter=0.22. Median signed radial error is 55.4 mm, median tangential magnitude is 40.9 mm, and 79% of points are radially outward.
...
Height grouping uses bootstrap CIs because each group is small. The output table reports 3D median/p95 and OptiTrack X/Y/Z split for the corrected all8 FULL run.
