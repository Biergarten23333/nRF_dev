...
def plot_axis_bias(path: Path, axis_df: pd.DataFrame) -> None:
    ...
    fig.suptitle("Production v4-io per-axis error, corrected full OptiTrack all8")
    fig.tight_layout()
    path.parent.mkdir(parents=True, exist_ok=True)
    fig.savefig(path)
    plt.close(fig)
...
def write_summary(path: Path, metrics: pd.DataFrame, axis_df: pd.DataFrame, outlier_df: pd.DataFrame) -> None:
    ...
    rows = []
    for eval_id in ["production_v4io_all8", "raw_v4io_T3_all8", "raw_v4io_T4_all8"]:
    ...
    axis_rows = []
    for eval_id in ["production_v4io_all8"]:
...
def main() -> None:
    ...
    parser.add_argument("--official-root", type=Path, default=Path(__file__).resolve().parents[4])
