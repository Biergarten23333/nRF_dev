...
def plot_matrix(summary_rows: list[dict], out: Path) -> None:
    ...
    fig, axs = plt.subplots(1, len(eval_sets), figsize=(6.0 * len(eval_sets), 4.5), constrained_layout=True, squeeze=False)
    ...
    if im is not None:
        fig.colorbar(im, ax=axs.ravel().tolist(), label="median 3D error mm")
