...
def save_fusion_comparison(directed):
    ...
    path = ROOT / "solves/fusion_comparison.csv"
...
def main():
    ...
    for cfg_name, anchor_ids in CONFIGS.items():
        ...
        for solver in SOLVERS:
            ...
            try:
                ...
                save_layout_json(ROOT / f"solves/{solver.replace('+','p').replace(' ','_').replace('-','_')}_{cfg_name.replace(' ','_').replace('+','p')}.json", solver, cfg_name, anchor_ids, x, dly, {"inter_rms": inter, **pos, "success": success}, extra)
    ...
    with (ROOT / "positioning/main_matrix.csv").open("w", newline="", encoding="utf-8") as f:
