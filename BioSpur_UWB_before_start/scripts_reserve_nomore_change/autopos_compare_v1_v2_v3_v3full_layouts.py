        ...
        for k, v in m.items():
            if k in ANCHORS and isinstance(v, list) and len(v) >= 2:
                out[k] = [
                    float(v[0]) * scale,
                    float(v[1]) * scale,
                    float(v[2] if len(v) > 2 else 0.0) * scale,
                ]
        return out
...
def kabsch_rigid_align(
    ...
    aligned: dict[str, list[float]] = {
        k: [float(x) for x in apply(a[k])] for k in a.keys() if k in ANCHORS
    }
    ...
    return aligned, {
        ...
        "max_m": float(mx),
        "n": float(len(errs)),
        **{f"err_{k}_m": float(per_anchor[k]) for k in per_anchor},
    }


def main() -> int:
    ap = argparse.ArgumentParser(description="Compare V1/V2/V3-lite/V3-full layouts with rigid alignment (Kabsch).")
    ap.add_argument("--v1", required=True, help="V1 layout json")
    ap.add_argument("--v2", required=True, help="V2 layout json")
    ap.add_argument("--v3", required=True, help="V3-lite layout json")
    ap.add_argument("--v3full", required=True, help="V3-full layout json")
    ...
    v4 = load_layout(Path(args.v3full))

    _, m21 = kabsch_rigid_align(v2, v1)
    _, m31 = kabsch_rigid_align(v3, v1)
    _, m41 = kabsch_rigid_align(v4, v1)
    _, m32 = kabsch_rigid_align(v3, v2)
    _, m42 = kabsch_rigid_align(v4, v2)
    _, m43 = kabsch_rigid_align(v4, v3)

    out = Path(args.out)
    out.parent.mkdir(parents=True, exist_ok=True)

    lines: list[str] = []
    lines.append("# AutoPos V1 / V2 / V3-lite / V3-full Layout Compare (Rigid-Aligned)")
    lines.append("")
    lines.append(f"- V1: `{Path(args.v1).resolve()}`")
    lines.append(f"- V2: `{Path(args.v2).resolve()}`")
    lines.append(f"- V3-lite: `{Path(args.v3).resolve()}`")
    lines.append(f"- V3-full: `{Path(args.v3full).resolve()}`")
    lines.append("")
    ...
    lines.append(f"| V3full aligned->V1 | {int(m41['n'])} | {m41['rms_m']:.6f} | {m41['max_m']:.6f} |")
    lines.append(f"| V3 aligned->V2 | {int(m32['n'])} | {m32['rms_m']:.6f} | {m32['max_m']:.6f} |")
    lines.append(f"| V3full aligned->V2 | {int(m42['n'])} | {m42['rms_m']:.6f} | {m42['max_m']:.6f} |")
    lines.append(f"| V3full aligned->V3 | {int(m43['n'])} | {m43['rms_m']:.6f} | {m43['max_m']:.6f} |")
    lines.append("")
    lines.append("## Notes")
