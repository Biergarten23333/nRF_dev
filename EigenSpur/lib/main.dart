...
const repoRoot = '/home/zekaixiao/Documents/nRF_dev/EigenSpur';
const biospurRepoRoot =
    '/home/zekaixiao/Documents/nRF_dev/BioSpur_UWB_before_start';
const fieldRoot = '$biospurRepoRoot/autopos_pipeline/erlangen_20260528_mocap';
...
String get workspaceSetup =>
    'cd $biospurRepoRoot && source $aliases && $workspaceExport && bio_setup $captureSession';
  ...
  Widget build(BuildContext context) {
    return Material(
      ...
      child: Stack(
        ...
        children: [
          Image.asset(
            'assets/images/eigenspur_audiopos_icon.png',
            fit: BoxFit.cover,
...
class EigenSpurPanningTab extends StatefulWidget {
  const EigenSpurPanningTab({
    super.key,
    required this.runner,
    required this.ports,
    required this.selectedLayoutPath,
  ...
  @override
  State<EigenSpurPanningTab> createState() => _EigenSpurPanningTabState();
}

class _EigenSpurPanningTabState extends State<EigenSpurPanningTab> {
  late final Timer _timer;
  final _inputDevice = TextEditingController();
  final _outputDevice = TextEditingController();
  final _blockSize = TextEditingController(text: '128');
  final _outputGain = TextEditingController(text: '2.0');
  final _captureId = TextEditingController(text: 'take01');
  double _previewU = 0.0;
  String _stageAxis = 'ab';
  bool _flipAxis = false;
  bool _saveCapture = false;
  EigenSpurMonitorSample? _sample;
  String _systemState = 'idle';

  bool get _running =>
      widget.runner.isRunning &&
      (widget.runner.activeName?.startsWith('EigenSpur ') ?? false);

  @override
  void initState() {
    super.initState();
    widget.runner.addListener(_refreshSample);
    _timer = Timer.periodic(
      const Duration(milliseconds: 250),
      (_) => _refreshSample(),
    );
  ...
  void dispose() {
    _timer.cancel();
    widget.runner.removeListener(_refreshSample);
  ...
  void _refreshSample() {
    final next = EigenSpurMonitorSample.fromLog(widget.runner.logTail);
    final nextState = _stateFromLog(widget.runner.logTail);
    if (next == _sample && nextState == _systemState || !mounted) return;
    setState(() {
      _sample = next;
      _systemState = nextState;
    });
  }

  String _stateFromLog(List<String> lines) {
    for (final line in lines.reversed) {
      if (line.contains('[system] normal')) return 'system normal';
      if (line.contains('[system] audio_started_waiting_for_uwb')) {
        return 'audio on, waiting UWB';
      }
      if (line.contains('[audio] stream started')) return 'audio on';
      if (line.contains('[capture] saving')) return 'recording';
    }
    if (_running) return 'starting';
    return 'idle';
  }

  Future<void> _start({required bool withUwb}) async {
    final script = '$repoRoot/scripts/eigenspur_stereo_panning.py';
    final args = <String>[
      'python3',
      shellQuote(script),
      '--source',
      withUwb ? 'serial' : 'none',
      '--stage-axis',
      shellQuote(_stageAxis),
      '--input-device',
      shellQuote(_inputDevice.text.trim()),
      '--output-device',
      shellQuote(_outputDevice.text.trim()),
      '--block-size',
      shellQuote(_blockSize.text.trim()),
      '--output-gain',
      shellQuote(_outputGain.text.trim()),
    ];
    if (withUwb) {
      args.addAll([
        '--serial-port',
        shellQuote(
          widget.ports.masterTagPath ??
              '/dev/serial/by-id/usb-Master_Tag_BioSpur_BLE_Control_6918E0384172A49F-if00',
        ),
      ]);
    }
    if (_flipAxis) args.add('--flip-axis');
    if (_saveCapture && _captureId.text.trim().isNotEmpty) {
      args.addAll(['--capture-id', shellQuote(_captureId.text.trim())]);
    }
    await widget.runner.start(
      withUwb ? 'EigenSpur Panning' : 'EigenSpur Audio Only',
      args.join(' '),
    );
  }
  ...
  Future<void> _listAudioDevices() async {
    await widget.runner.start(
      'EigenSpur Audio Devices',
      'python3 ${shellQuote('$repoRoot/scripts/eigenspur_stereo_panning.py')} --list-devices',
    );
  }
  ...
  Widget build(BuildContext context) {
    const left = -1.0;
    const right = 1.0;
    final displayU = _sample?.u ?? (_flipAxis ? -_previewU : _previewU);
    final t = ((displayU - left) / (right - left)).clamp(0.0, 1.0);
    final leftGain = _sample?.leftGain ?? (1.0 - t);
    final rightGain = _sample?.rightGain ?? t;
    return ListView(
      padding: const EdgeInsets.all(16),
      children: [
        Card(
              ...
              children: [
                Row(
                  children: [
                    ...
                    StatusPill(
                      label: 'State',
                      value: _systemState,
                      tone: _systemState == 'system normal'
                          ? PillTone.good
                          : _running
                          ? PillTone.active
                          : PillTone.neutral,
                ...
                LayoutBuilder(
                  builder: (context, constraints) {
                    ...
                    final controls = _PanningControls(
                      stageAxis: _stageAxis,
                      flipAxis: _flipAxis,
                      saveCapture: _saveCapture,
                      tagCdcReady: widget.ports.masterTag,
                      inputDevice: _inputDevice,
                      outputDevice: _outputDevice,
                      blockSize: _blockSize,
                      outputGain: _outputGain,
                      captureId: _captureId,
                      onStageAxisChanged: (value) =>
                          setState(() => _stageAxis = value),
                      onFlipAxisChanged: (value) =>
                          setState(() => _flipAxis = value),
                      onSaveCaptureChanged: (value) =>
                          setState(() => _saveCapture = value),
                    );
                    final monitor = EigenSpurStageMonitor(
                      sample: _sample,
                      stageAxis: _stageAxis,
                      flipAxis: _flipAxis,
                      layoutPath: widget.selectedLayoutPath,
                      previewU: _previewU,
                      onPreviewUChanged: (value) {
                        setState(() => _previewU = value);
                      },
                    );
                    final gain = _AudioGainPanel(
                      sample: _sample,
                      leftGain: leftGain,
                      rightGain: rightGain,
                    );
                      ...
                      return Column(
                        children: [
                          _ControlBlock(
                            controls: controls,
                            runner: widget.runner,
                            onStartAudio: () => _start(withUwb: false),
                            onStartAudioUwb: () => _start(withUwb: true),
                            onTestOutput: _testOutput,
                            onListAudioDevices: _listAudioDevices,
                          ),
                          const SizedBox(height: 12),
                          SizedBox(height: 360, child: monitor),
                          const SizedBox(height: 12),
                          gain,
                        ],
                      );
                      ...
                      children: [
                        ...
                        Row(
                          ...
                          children: [
                            Expanded(
                              flex: 7,
                              child: SizedBox(height: 430, child: monitor),
                            ),
                            const SizedBox(width: 14),
                            Expanded(flex: 3, child: gain),
                          ],
                        ),
                      ],
                    );
...
class _PanningControls extends StatelessWidget {
  const _PanningControls({
    required this.stageAxis,
    required this.flipAxis,
    required this.saveCapture,
    required this.tagCdcReady,
    required this.inputDevice,
    required this.outputDevice,
    required this.blockSize,
    required this.outputGain,
    required this.captureId,
    required this.onStageAxisChanged,
    required this.onFlipAxisChanged,
    required this.onSaveCaptureChanged,
  });

  final String stageAxis;
  final bool flipAxis;
  final bool saveCapture;
  final bool tagCdcReady;
  final TextEditingController inputDevice;
  final TextEditingController outputDevice;
  final TextEditingController blockSize;
  final TextEditingController outputGain;
  final TextEditingController captureId;
  final ValueChanged<String> onStageAxisChanged;
  final ValueChanged<bool> onFlipAxisChanged;
  final ValueChanged<bool> onSaveCaptureChanged;
      ...
      children: [
        Wrap(
          ...
          children: [
            FilterChip(
              ...
              label: const Text('BSF66F'),
              selected: true,
              onSelected: (_) {},
            ),
            StatusPill(
              label: 'Tag CDC',
              value: tagCdcReady ? 'OK' : 'missing',
              tone: tagCdcReady ? PillTone.good : PillTone.bad,
            ),
            SegmentedButton<String>(
              segments: const [
                ButtonSegment(value: 'ab', label: Text('AB')),
                ButtonSegment(value: 'bc', label: Text('BC')),
                ButtonSegment(value: 'cd', label: Text('CD')),
                ButtonSegment(value: 'da', label: Text('DA')),
              ],
              selected: {stageAxis},
              onSelectionChanged: (value) => onStageAxisChanged(value.first),
            ),
            FilterChip(
              avatar: const Icon(Icons.swap_horiz, size: 18),
              label: const Text('Flip +/-'),
              selected: flipAxis,
              onSelected: onFlipAxisChanged,
            ),
            FilterChip(
              avatar: const Icon(Icons.fiber_manual_record, size: 18),
              label: const Text('Save WAV + UWB CSV'),
              selected: saveCapture,
              onSelected: onSaveCaptureChanged,
            ),
          ],
        ),
        const SizedBox(height: 12),
        Wrap(
          ...
          children: [
            ...
            _AudioDeviceDropdown(
              controller: outputDevice,
              label: 'Audio output',
              kind: AudioDeviceKind.output,
              width: 360,
            ),
            _FieldBox(controller: blockSize, label: 'Block size', width: 130),
            _FieldBox(controller: outputGain, label: 'Output gain', width: 130),
            if (saveCapture)
              _FieldBox(controller: captureId, label: 'Capture ID', width: 180),
          ],
        ),
      ],
    );
...
class _ControlBlock extends StatelessWidget {
  const _ControlBlock({
    ...
    required this.onStartAudioUwb,
    required this.onTestOutput,
  ...
  final VoidCallback onStartAudioUwb;
  final VoidCallback onTestOutput;
      ...
      child: Column(
        ...
        children: [
          ...
          Wrap(
            ...
            children: [
              FilledButton.icon(
                onPressed: runner.isRunning ? null : onStartAudio,
                icon: const Icon(Icons.volume_up_outlined),
                label: const Text('Start Audio'),
              ),
              FilledButton.icon(
                onPressed: runner.isRunning ? null : onStartAudioUwb,
                icon: const Icon(Icons.play_arrow),
                label: const Text('Start Audio + UWB'),
              ),
              OutlinedButton.icon(
                onPressed: runner.isRunning ? runner.stop : null,
                icon: const Icon(Icons.stop),
              ...
              TextButton.icon(
                onPressed: runner.isRunning ? null : onTestOutput,
                icon: const Icon(Icons.graphic_eq),
                label: const Text('Test Output'),
              ),
              TextButton.icon(
                onPressed: runner.isRunning ? null : onListAudioDevices,
...
class AudioDeviceOption {
  const AudioDeviceOption({required this.name, required this.label});

  final String name;
  final String label;
}

class _AudioDeviceDropdown extends StatefulWidget {
  ...
  @override
  State<_AudioDeviceDropdown> createState() => _AudioDeviceDropdownState();
}

class _AudioDeviceDropdownState extends State<_AudioDeviceDropdown> {
  late Future<List<AudioDeviceOption>> _future;
  List<AudioDeviceOption> _options = const [];

  @override
  void initState() {
    super.initState();
    _future = AudioDeviceReader.read(widget.kind);
  }
  ...
  Widget build(BuildContext context) {
    return SizedBox(
      width: widget.width,
      child: FutureBuilder<List<AudioDeviceOption>>(
        future: _future,
        builder: (context, snapshot) {
          _options = snapshot.data ?? _options;
          final selected = _selectedOption;
          final menuWidth = widget.width - 42;
          return Row(
            children: [
              SizedBox(
                width: menuWidth,
                child: AnchoredInlineMenu(
                  width: menuWidth,
                  label: widget.label,
                  value: selected?.label ?? 'Select ${widget.label.toLowerCase()}',
                  enabled:
                      snapshot.connectionState == ConnectionState.done &&
                      _options.isNotEmpty,
                  menuHeight: math.min(220.0, 44.0 * math.max(1, _options.length)),
                  itemBuilder: (close) => [
                    for (final option in _options)
                      InlineMenuItem(
                        selected: option.name == selected?.name,
                        icon: widget.kind == AudioDeviceKind.input
  ...
  AudioDeviceOption? get _selectedOption {
    for (final option in _options) {
      if (option.name == widget.controller.text) return option;
    }
    if (_options.isEmpty) return null;
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (!mounted || widget.controller.text == _options.first.name) return;
      setState(() => widget.controller.text = _options.first.name);
    });
...
class AudioDeviceReader {
  static Future<List<AudioDeviceOption>> read(AudioDeviceKind kind) async {
    final result = await Process.run('python3', [
      '-c',
      '''
import json
...
for i, d in enumerate(sd.query_devices()):
    if ${kind == AudioDeviceKind.output ? 'True' : 'False'}:
        channels = int(d.get("max_output_channels") or 0)
    else:
        channels = int(d.get("max_input_channels") or 0)
    if channels <= 0:
        continue
    out.append({"index": str(i), "name": d.get("name", str(i)), "channels": channels})
print(json.dumps(out))
''',
    ]);
    if (result.exitCode != 0) return const [];
    final out = <AudioDeviceOption>[];
    try {
      ...
      for (final item in decoded) {
        ...
        final rawName = item['name']?.toString() ?? index;
        final channels = item['channels']?.toString() ?? '';
        if (index == null || rawName == null) continue;
        out.add(
          AudioDeviceOption(
            name: rawName,
            label: _friendlySoundDeviceLabel(rawName, channels, kind),
          ),
        );
      }
    } catch (_) {
    ...
    out.sort((a, b) {
      int score(AudioDeviceOption option) {
        final lower = option.label.toLowerCase();
        if (lower.contains('system default')) {
          return kind == AudioDeviceKind.output ? 0 : 1;
        }
        if (lower.contains('voicelive')) {
          return kind == AudioDeviceKind.input ? 0 : 3;
        }
        if (lower.contains('line out')) return 1;
        if (lower.contains('pipewire')) return 2;
        if (lower.contains('built-in')) return 3;
        return 3;
      }

      final byScore = score(a).compareTo(score(b));
      return byScore != 0 ? byScore : a.label.compareTo(b.label);
    });
    return out;
  }

  static String _friendlySoundDeviceLabel(
    String rawName,
    String channels,
    AudioDeviceKind kind,
  ) {
    if (rawName.contains('VoiceLive Play')) {
      return 'VoiceLive Play Analog Stereo';
    }
    if (rawName == 'default') {
      return kind == AudioDeviceKind.input
          ? 'System Default Input'
          : 'Line Out - Built-in Audio (system default)';
    }
    if (rawName == 'pipewire') {
      return kind == AudioDeviceKind.input
          ? 'PipeWire Input'
          : 'PipeWire Output (Ubuntu selected)';
    }
    if (rawName.contains('ALC1220 Analog') && kind == AudioDeviceKind.output) {
      return 'Line Out - Built-in Audio';
    }
    if (rawName.contains('ALC1220 Analog') && kind == AudioDeviceKind.input) {
      return 'Built-in Audio Analog Stereo';
    }
    return '$rawName ($channels ch)';
  }
}

class EigenSpurMonitorSample {
  const EigenSpurMonitorSample({
    required this.x,
    required this.y,
    required this.z,
    required this.u,
    ...
    required this.outputLeftLevel,
    required this.outputRightLevel,
  });
  ...
  final double outputLeftLevel;
  final double outputRightLevel;

  static EigenSpurMonitorSample? fromLog(List<String> lines) {
    for (final line in lines.reversed) {
      if (!line.contains('outR=') || !line.contains('in=')) continue;
      double field(String name) {
        final match = RegExp('$name=\\s*(-?[0-9.]+)').firstMatch(line);
        return double.tryParse(match?.group(1) ?? '') ?? 0.0;
      }

      return EigenSpurMonitorSample(
        x: field('x'),
        y: field('y'),
        z: field('z'),
        u: field('u'),
        ...
        rightGain: field('R'),
        inputLevel: field('in'),
        outputLeftLevel: field('outL'),
        outputRightLevel: field('outR'),
      );
    }
    return null;
  }
  ...
  bool operator ==(Object other) =>
      other is EigenSpurMonitorSample &&
      other.x == x &&
      other.y == y &&
      other.z == z &&
      other.u == u &&
      other.v == v &&
      other.leftGain == leftGain &&
      other.rightGain == rightGain &&
      other.inputLevel == inputLevel &&
      other.outputLeftLevel == outputLeftLevel &&
      other.outputRightLevel == outputRightLevel;
...
class EigenSpurStageMonitor extends StatelessWidget {
  const EigenSpurStageMonitor({
    super.key,
    required this.sample,
    required this.stageAxis,
    required this.flipAxis,
    required this.layoutPath,
    required this.previewU,
    required this.onPreviewUChanged,
  });

  final EigenSpurMonitorSample? sample;
  final String stageAxis;
  final bool flipAxis;
  final String? layoutPath;
  final double previewU;
  final ValueChanged<double> onPreviewUChanged;
        ...
        children: [
          Padding(
            ...
            child: Row(
              children: [
                Text(
                  'UWB Position Monitor (${stageAxis.toUpperCase()}${flipAxis ? ' flipped' : ''})',
                  style: const TextStyle(
                    fontSize: 17,
                    fontWeight: FontWeight.w800,
                  ),
                ...
                StatusPill(
                  label: 'Tag',
                  value: sample == null ? 'preview' : 'live',
                  tone: sample == null ? PillTone.neutral : PillTone.good,
                ),
              ],
            ),
          ),
          Expanded(
            child: FutureBuilder<AnchorLayoutData?>(
              future: EigenSpurLayoutLoader.read(layoutPath),
              builder: (context, snapshot) {
                final layout = snapshot.data;
                final data = _trajectoryData(layout);
                return Trajectory3DView(
                  data: data,
                  layout: layout,
                  visibleTags: data.tags.toSet(),
                  currentTimeS: 0,
                  compactLabel: sample == null
                      ? 'BSF66F preview'
                      : 'BSF66F realtime',
                  displayMode: 'dots',
                  dotSize: sample == null ? 8 : 11,
                  stageAxisOverlay: stageAxis,
                  stageAxisFlip: flipAxis,
                );
              },
            ),
          ),
  ...
  TrajectoryData _trajectoryData(AnchorLayoutData? layout) {
    final frame = sample == null
        ? TrajectoryFrame(
            tag: 'BSF66F',
            sweep: 0,
            hostElapsedS: 0,
            xMm: layout?.centerX ?? 0,
            yMm: layout?.centerY ?? 0,
            zMm: layout?.centerZ ?? 0,
            anchorsUsed: 0,
            residualRmsMm: 0,
          )
        : TrajectoryFrame(
    ...
    return TrajectoryData(
      sourcePath: sample == null ? 'preview / waiting UWB' : 'live UWB',
      candidateFrames: 1,
      solvedFrames: sample == null ? 0 : 1,
      expectedTags: const ['BSF66F'],
      candidateFramesByTag: const {'BSF66F': 1},
      solvedFramesByTag: {'BSF66F': sample == null ? 0 : 1},
      tags: const ['BSF66F'],
      frames: [frame],
    );
  }
    ...
    return Row(
      children: [
        ...
        Expanded(
          child: SizedBox(
            height: 28,
            child: _ClassicHorizontalBar(
              value: clamped,
              peakValue: peakValue?.clamp(0.0, 1.0),
            ),
          ),
        ),
      ],
...
class _ClassicHorizontalBar extends StatelessWidget {
  const _ClassicHorizontalBar({required this.value, this.peakValue});
  ...
  Widget build(BuildContext context) {
    const segments = 28;
    final peakIndex = peakValue == null
        ? -1
        : ((peakValue! * segments).ceil() - 1).clamp(0, segments - 1);
    final fill = value.clamp(0.0, 1.0);
    return Container(
      padding: const EdgeInsets.all(3),
      decoration: BoxDecoration(
        color: const Color(0xFF181B16),
        border: Border.all(color: const Color(0xFF38432D)),
...
class _AudioGainPanel extends StatelessWidget {
  const _AudioGainPanel({
    required this.sample,
    required this.leftGain,
    required this.rightGain,
  });
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              const Icon(Icons.equalizer, color: controlGreen, size: 18),
              const SizedBox(width: 8),
              const Expanded(
                child: Text(
                  'Audio / Gain Monitor',
          ...
          _LevelMeter(
            label: 'Mic input',
            value: _audioMeterValue(sample?.inputLevel ?? 0.0),
            peakValue: _audioPeakValue(sample?.inputLevel ?? 0.0),
          ),
          const SizedBox(height: 14),
          _LevelMeter(label: 'Left gain', value: sample?.leftGain ?? leftGain),
          const SizedBox(height: 14),
          _LevelMeter(
            label: 'Right gain',
            value: sample?.rightGain ?? rightGain,
          ),
          const SizedBox(height: 14),
          _LevelMeter(
            label: 'Left output',
            value: _audioMeterValue(sample?.outputLeftLevel ?? 0.0),
            peakValue: _audioPeakValue(sample?.outputLeftLevel ?? 0.0),
          ),
          const SizedBox(height: 14),
          _LevelMeter(
            label: 'Right output',
            value: _audioMeterValue(sample?.outputRightLevel ?? 0.0),
            peakValue: _audioPeakValue(sample?.outputRightLevel ?? 0.0),
          ),
        ],
      ),
    );
  ...
  double _audioMeterValue(double rms) {
    if (rms <= 0.00001) return 0.0;
    final db = 20.0 * math.log(rms) / math.ln10;
    return ((db + 62.0) / 42.0).clamp(0.0, 1.0);
  }

  double _audioPeakValue(double rms) {
    if (rms <= 0.00001) return 0.0;
    final db = 20.0 * math.log(rms * 1.8) / math.ln10;
    return ((db + 62.0) / 42.0).clamp(0.0, 1.0);
  }
}

class EigenSpurLayoutLoader {
  static Future<AnchorLayoutData?> read(String? preferredPath) async {
    if (preferredPath != null && preferredPath.isNotEmpty) {
      final selected = await AnchorLayoutData.read(
        'selected',
        File(preferredPath),
      );
    ...
    for (final file in candidates) {
      ...
      final layout = await AnchorLayoutData.read('eigenspur', file);
...
class EigenSpurStagePainter extends CustomPainter {
  const EigenSpurStagePainter({
    required this.sample,
    required this.layout,
    required this.stageAxis,
    required this.flipAxis,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final grid = Paint()
      ..color = controlGreen.withValues(alpha: 0.18)
      ..strokeWidth = 1;
    for (var i = 1; i < 6; i++) {
    ...
    final points = layout?.points ?? const <AnchorPoint>[];
    final xs = points.map((p) => p.x / 1000.0).toList();
    final ys = points.map((p) => p.y / 1000.0).toList();
    if (sample != null) {
      xs.add(sample!.x);
      ys.add(sample!.y);
    }
    ...
    final spanX = math.max(0.5, maxX - minX);
    final spanY = math.max(0.5, maxY - minY);
    final padX = spanX * 0.18;
    final padY = spanY * 0.18;

    Offset map(double xM, double yM) {
      final x =
          ((xM - minX + padX) / (spanX + padX * 2)).clamp(0.0, 1.0) *
          size.width;
      final y =
          size.height -
          ((yM - minY + padY) / (spanY + padY * 2)).clamp(0.0, 1.0) *
              size.height;
      return Offset(x, y);
    }

    void label(String text, Offset p, TextStyle style) {
    ...
    final byLabel = layout?.byLabel ?? const <String, AnchorPoint>{};
    final edgeLabels = switch (stageAxis) {
      'bc' => const ['B', 'C'],
      'cd' => const ['C', 'D'],
      'da' => const ['D', 'A'],
      _ => const ['A', 'B'],
    ...
    final linePaint = Paint()
      ..color = controlGreen.withValues(alpha: 0.40)
      ..strokeWidth = 1.3;
    void connect(String a, String b) {
      ...
      if (pa == null || pb == null) return;
      canvas.drawLine(
        map(pa.x / 1000.0, pa.y / 1000.0),
        map(pb.x / 1000.0, pb.y / 1000.0),
        linePaint,
      );
    }
    ...
    for (final anchor in points) {
      final p = map(anchor.x / 1000.0, anchor.y / 1000.0);
      final fill = Paint()..color = const Color(0xFFE8FF4F);
      final stroke = Paint()
        ..color = Colors.white
        ..style = PaintingStyle.stroke
        ..strokeWidth = 1.2;
    ...
    final a0 = byLabel[edgeLabels[0]];
    final a1 = byLabel[edgeLabels[1]];
    Offset axisStart;
    Offset axisEnd;
    if (a0 != null && a1 != null) {
      axisStart = map(a0.x / 1000.0, a0.y / 1000.0);
      axisEnd = map(a1.x / 1000.0, a1.y / 1000.0);
    } else {
      axisStart = map(minX, (minY + maxY) / 2);
      axisEnd = map(maxX, (minY + maxY) / 2);
    }
    canvas.drawLine(axisStart, axisEnd, axisPaint);

    final axis = stageAxis.toUpperCase();
    final leftLabel = flipAxis ? 'R' : 'L';
    final rightLabel = flipAxis ? 'L' : 'R';
    label(
      leftLabel,
      axisStart + const Offset(-18, -28),
      const TextStyle(
    ...
    final p = sample == null
        ? map((minX + maxX) / 2, (minY + maxY) / 2)
        : map(sample!.x, sample!.y);
    if (sample == null) {
      canvas.drawCircle(
        p,
        9,
        Paint()..color = mutedText.withValues(alpha: 0.55),
      );
      label(
        layout == null ? 'waiting layout / UWB' : 'waiting UWB',
        p + const Offset(12, -8),
        const TextStyle(color: mutedText, fontSize: 12),
      );
    } else {
      canvas.drawCircle(p, 10, Paint()..color = biospurGlow);
      canvas.drawCircle(
        p,
        18,
        Paint()
          ..color = biospurGlow.withValues(alpha: 0.22)
          ..style = PaintingStyle.fill,
      );
      label(
        'x ${sample!.x.toStringAsFixed(2)}  y ${sample!.y.toStringAsFixed(2)}  z ${sample!.z.toStringAsFixed(2)}',
        const Offset(12, 12),
        const TextStyle(color: mutedText, fontSize: 12),
      );
    }
  ...
  @override
  bool shouldRepaint(covariant EigenSpurStagePainter oldDelegate) =>
      oldDelegate.sample != sample ||
      oldDelegate.layout != layout ||
      oldDelegate.stageAxis != stageAxis ||
      oldDelegate.flipAxis != flipAxis ||
      oldDelegate.previewU != previewU;
}
...
class Trajectory3DView extends StatefulWidget {
  const Trajectory3DView({
    ...
    this.stageAxisOverlay,
    this.stageAxisFlip = false,
  });
  ...
  final String? stageAxisOverlay;
  final bool stageAxisFlip;
                    ...
                    Positioned.fill(
                      child: CustomPaint(
                        painter: TrajectoryPainter(
                          ...
                          stageAxisOverlay: widget.stageAxisOverlay,
                          stageAxisFlip: widget.stageAxisFlip,
                        ),
                        child: const SizedBox.expand(),
                      ),
                    ),
...
class TrajectoryPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (layout != null) {
      ...
      final overlay = stageAxisOverlay;
      if (overlay != null) {
        final labels = switch (overlay) {
          'bc' => const ['B', 'C'],
          'cd' => const ['C', 'D'],
          'da' => const ['D', 'A'],
          _ => const ['A', 'B'],
        };
        final pa = layout!.byLabel[labels[0]];
        final pb = layout!.byLabel[labels[1]];
        if (pa != null && pb != null) {
          final a = project(pa.x, pa.y, pa.z);
          final b = project(pb.x, pb.y, pb.z);
          canvas.drawLine(
            a,
            b,
            Paint()
          ...
          label(
            stageAxisFlip ? 'R' : 'L',
            a + const Offset(-20, -28),
            const Color(0xFFFF3B30),
          );
          label(
            stageAxisFlip ? 'L' : 'R',
            b + const Offset(10, -28),
            const Color(0xFFFF3B30),
          );
        }
      }
...
class ScriptRunner extends ChangeNotifier {
  ...
  Future<void> stop() async {
    ...
    progressValue = null;
    progressText = 'stopping current command';
    _append('[ui] stop requested for current command only');
    notifyListeners();
    if (process == null) {
      progressText = 'no running command';
      _append('[ui] no active command process; nothing killed');
      notifyListeners();
      return;
    }
...
class PortReader {
  static Future<PortSnapshot> read() async {
    ...
    final names = entries.map((e) => e.path.split('/').last).join('\n');
    String? findPath(String token) {
      for (final entry in entries) {
        final name = entry.path.split('/').last;
        if (name.contains(token)) return entry.path;
      }
      return null;
    }

    final anchorPath = findPath('BioSpur_BioSpur_BLE_Control_87EA2F4A526C5A02');
    final tagPath = findPath('Master_Tag_BioSpur_BLE_Control_6918E0384172A49F');
    return PortSnapshot(
      ...
      masterAnchorPath: anchorPath,
      masterTagPath: tagPath,
    );
  }
}
...
class PortSnapshot {
  const PortSnapshot({
    ...
    this.masterAnchorPath,
    this.masterTagPath,
  });

  final bool masterAnchor;
  final bool masterTag;
  final String? masterAnchorPath;
  final String? masterTagPath;
