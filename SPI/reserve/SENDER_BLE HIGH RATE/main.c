...
#define ECG_RATE_HZ     150
#define ECG_INT_US      (1000000U / ECG_RATE_HZ)   /* ≈ 6666 µs */
#define ECG_PKT_N       5                           /* 每包 5 个样本 */
...
static void ecg_thread(void *a, void *b, void *c)
{
    ...
    gpio_pin_configure(gpio_ecg, 4, GPIO_INPUT | GPIO_PULL_UP);
    gpio_pin_configure(gpio_ecg, 5, GPIO_INPUT | GPIO_PULL_UP);

    /* ── SAADC 配置 ── */
    if (!adc_is_ready_dt(&adc)) { printk("ADC not ready\n"); return; }
    if (adc_channel_setup_dt(&adc)) { printk("ADC setup fail\n"); return; }

    /* 异步序列，用一个 buf[5] 双缓冲 */
    static int16_t buf[ECG_PKT_N];

    static struct k_poll_signal done_sig;
    k_poll_signal_init(&done_sig);
    struct k_poll_event done_evt = K_POLL_EVENT_INITIALIZER(
        K_POLL_TYPE_SIGNAL, K_POLL_MODE_NOTIFY_ONLY, &done_sig);

    struct adc_sequence_options opts = {
        .interval_us     = ECG_INT_US,        /* 6 666 µs/点 = 150 Hz */
        .callback        = NULL,
        .extra_samplings = ECG_PKT_N - 1,     /* 再采 4 次 → 共 5 点 */
    };
    ...
    printk("ECG ready (async %d × %d Hz)\n", ECG_PKT_N, ECG_RATE_HZ);
    ...
    while (1) {
        ...
        /* 2) 触发一次异步采样 → 5 点自动完成 */
        if (adc_read_async(adc.dev, &seq, &done_sig) < 0) {
            printk("ADC start fail\n");
            continue;
        }

        /* 3) 等待 SAADC END IRQ 完成（内核会自动 k_poll_signal_raise） */
        k_poll(&done_evt, 1, K_FOREVER);

        /* 4) 打包 5 点文本发送 */
        char line[96];
        int pos = snprintk(line, sizeof(line), "[%d", buf[0]);
        for (int i = 1; i < ECG_PKT_N; i++) {
            pos += snprintk(line + pos, sizeof(line) - pos, ",%d", buf[i]);
        }
        ...
        unsigned int sig;
        int res;
        k_poll_signal_check(&done_sig, &sig, &res);
    }
}
